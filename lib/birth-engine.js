...
// ※ 접두어로 붙이면 뒤 문장과 안 맞물림('다 해주려는 마음이 앞서 신나게 놀아주다'처럼) → 독립 문장으로 분리
const STR_OVER={
 신강:'한번 나서면 세게 밀어붙일 수 있어요.',
 중화신강:'추진력이 좋아 아이보다 속도가 앞설 수 있어요.',
 중화신약:'',
 신약:'아이에게 맞추다 정작 본인이 지치기 쉬워요.',
 태약:'애쓰는 마음이 큰 만큼 쉽게 소진되니, 부모 자신도 돌봐주세요.',
};
...
function analyzeParent(pc){
  ...
  const _lvNote=STR_OVER[level] ? ' '+STR_OVER[level] : '';
  const watch=`이럴 때 과해져요 — ${prof.over}.${_lvNote} ▷부모 미션 — ${prof.mission}`;
