...
function analyzeParent(pc){
  ...
  const love=`${A.love} ${BR_NUANCE[brSS]||''}`;
  const arche=`${A.arche} · ${BR_SUB[brSS]||''}`;
  return { dayPillar:TG[dayTG]+pc.dayBranch, dayEl:pc.dayEl, level, yong, hee, sipseongTop:top, branchSipseong:brSS, dist, arche, trait, love, watch:A.watch };
}
