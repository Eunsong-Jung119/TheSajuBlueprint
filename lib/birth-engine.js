...
function analyzeParent(pc){
  ...
  return { dayPillar:TG[dayTG]+pc.dayBranch, dayEl:pc.dayEl, level:ratioLv(pr), yong, hee, sipseongTop:top, dist, ...PARENT_ARCHE[top] };
}
// ── 아기 → 부모 인연 (아기가 부모에게 채워주는 기운) ──
function babyToParent(babyDayEl, babyPct, parentAn){
  const gen={목:'화',화:'토',토:'금',금:'수',수:'목'};        // A생B
  const py=parentAn.yong;
  // 아기가 부모 용신을 얼마나 지녔나 (일간 오행 or 오행 분포)
  ...
  if(hasYong){ kind='용신보충';
    text=`제 안의 <b>${py}(${HZ_[py]})</b> 기운이 마침 {부모}에게 꼭 필요한 기운이에요. 제가 곁에 오면 {부모}의 부족한 자리가 채워지는, 서로에게 힘이 되는 인연이에요.`; }
  else if(babyFeedsParent){ kind='생조';
    text=`제 <b>${babyDayEl}(${HZ_[babyDayEl]})</b> 기운이 {부모}의 기운을 살려주는 결이에요(${babyDayEl}생${parentAn.dayEl}). 제가 {부모}를 키우는 건 아니지만, {부모}가 지친 자리를 은근히 북돋아 드려요.`; }
  else { kind='비화조화';
    text=`저와 {부모}는 결이 닮아, 함께 있으면 마음이 편안해지는 인연이에요. 굳이 애쓰지 않아도 서로 자연스럽게 스며드는 사이예요.`; }
  return { kind, text };
}
