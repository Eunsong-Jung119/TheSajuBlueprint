...
const ZHI_WX_=['수','토','목','목','토','화','화','토','금','금','토','수'];  // 자축인묘진사오미신유술해
...
// ── 일지 십성 10종 레이어 ──
// 5종 카테고리(비겁/식상/재성/관성/인성)로만 가르면 정재와 편재가 같은 칸에 들어가,
// 엄마(壬午=정재)와 아빠(乙未=편재)가 토씨 하나 안 틀리고 같은 문장으로 나왔음.
// → 일지는 10종 그대로 써서 헤드라인·사랑의언어·본문을 가른다. (5 × 10 = 50조합)
const HEAD10={
 비겁:{비견:'끝까지 나란히 함께 가는 동지형 부모', 겁재:'서로 밀어주며 같이 달리는 동지형 부모',
       식신:'친구처럼 어울리며 함께 웃는 부모', 상관:'대등하게 티격태격하며 크는 부모',
       정재:'나란히 서되 필요한 건 야무지게 챙기는 부모', 편재:'함께 뛰면서 시원하게 안겨주는 부모',
       정관:'친구 같으면서도 중심은 분명한 부모', 편관:'편하게 지내다가도 결정적일 땐 강단 있는 부모',
       정인:'동지처럼 곁에 서서 깊이 품어주는 부모', 편인:'나란히 걸으며 속을 꿰뚫어 보는 부모'},
 식상:{비견:'같이 놀며 대등하게 어울리는 부모', 겁재:'몸으로 부딪히며 신나게 노는 부모',
       식신:'웃음이 끊이지 않는 놀이 짝꿍 부모', 상관:'재치로 아이를 웃기는 장난꾸러기 부모',
       정재:'신나게 놀아주면서 실속도 챙기는 부모', 편재:'판을 크게 벌여 놀아주는 부모',
       정관:'즐겁게 놀되 약속은 지키게 하는 부모', 편관:'실컷 놀리다가도 선은 단호히 긋는 부모',
       정인:'함께 놀며 마음까지 읽어주는 부모', 편인:'놀이 속에서 아이의 속내를 알아채는 부모'},
 재성:{비견:'필요한 걸 챙기며 나란히 함께 뛰는 부모', 겁재:'챙겨주면서 같이 부딪혀주는 부모',
       식신:'야무지게 챙기면서 편하게 놀아주는 부모', 상관:'살뜰히 챙기며 재치로 웃겨주는 부모',
       정재:'빈틈없이 미리 챙기는 살림 실전형 부모', 편재:'필요한 건 시원하게 안겨주는 손 큰 부모',
       정관:'챙길 건 챙기고 지킬 선은 지키는 부모', 편관:'다 챙겨주되 기준 앞에선 단호한 부모',
       정인:'현실을 챙기면서 마음까지 살피는 부모', 편인:'말없이 챙기며 속을 먼저 읽는 부모'},
 관성:{비견:'기준을 세우되 눈높이를 맞추는 부모', 겁재:'원칙을 지키며 같이 뛰어주는 부모',
       식신:'규칙 안에서 마음껏 놀게 해주는 부모', 상관:'틀은 잡되 엉뚱함은 살려주는 부모',
       정재:'울타리를 세우고 살뜰히 챙기는 부모', 편재:'큰 틀만 잡고 나머지는 넉넉히 맡기는 부모',
       정관:'한결같은 리듬으로 지켜주는 원칙형 부모', 편관:'분명한 기준으로 중심을 잡아주는 부모',
       정인:'기준은 분명하되 속마음을 품어주는 부모', 편인:'원칙을 지키며 깊이 헤아리는 부모'},
 인성:{비견:'마음을 읽어주며 나란히 걷는 부모', 겁재:'품어주면서 함께 부딪혀주는 부모',
       식신:'품어주면서 표현을 이끌어내는 부모', 상관:'깊이 품으며 재치를 키워주는 부모',
       정재:'마음을 품으며 필요한 걸 챙겨주는 부모', 편재:'넉넉히 품고 아낌없이 내주는 부모',
       정관:'깊이 이해하되 지킬 선은 분명한 부모', 편관:'품어주다가도 필요할 땐 단단해지는 부모',
       정인:'끝까지 기다려주는 정서적 안전기지', 편인:'말하지 않아도 먼저 알아채는 부모'},
};
// 사랑의 언어 3번째 항목 — 일지 10종
const LANG_BR10={
 비견:'눈높이 맞추기', 겁재:'같이 부딪혀주기',
 식신:'함께 웃어주기', 상관:'재치로 풀어주기',
 정재:'빠짐없이 챙기기', 편재:'아낌없이 내주기',
 정관:'든든한 울타리 되어주기', 편관:'중심 잡아주기',
 정인:'마음 먼저 헤아리기', 편인:'말 없이 알아채기',
};
// 본문에 덧붙는 일지 10종 결 — 같은 카테고리라도 정/편이 다르면 여기서 갈린다
const BR10_TOUCH={
 비견:'아이를 아랫사람이 아니라 한 사람으로 대해, 집에서도 서로 할 말은 하는 편이에요.',
 겁재:'아이와 부딪히는 걸 겁내지 않아, 같이 씨름하다 금세 풀어지는 사이가 돼요.',
 식신:'곁에 있으면 편안해지는 분위기라, 아이가 눈치 보지 않고 제 기분을 내보여요.',
 상관:'할 말을 돌려 말하지 않아 솔직하고, 그만큼 아이도 제 생각을 숨기지 않아요.',
 정재:'챙기는 방식이 꼼꼼하고 일정해서, 아이가 예측 가능한 안정감 속에서 자라요.',
 편재:'주저 없이 크게 내주는 편이라, 아이가 부족함을 모르고 넉넉하게 자라요.',
 정관:'말과 행동이 한결같아, 아이가 무엇을 기대해도 되는지 분명히 알아요.',
 편관:'평소엔 무던하다가 중요한 순간 단호해져서, 아이가 선을 또렷이 배워요.',
 정인:'아이 기분의 결을 남보다 빨리 알아채, 묻기 전에 먼저 다가가요.',
 편인:'겉으로 드러난 말보다 그 뒤의 속내를 읽어내, 아이가 이해받는다고 느껴요.',
};
...
// 부십성이 top·일지와 겹쳐 쓸 수 없을 때 대신 붙는 신강약 결 (부모끼리 신강약이 다르면 여기서 갈림)
const LEVEL_TONE={
 신강:'한번 마음먹은 방식은 좀처럼 바꾸지 않는 뚝심이 있어요.',
 중화신강:'심지가 단단해 아이 앞에서 쉽게 흔들리지 않아요.',
 중화신약:'아이의 기분 변화를 남보다 섬세하게 알아채는 편이에요.',
 신약:'아이에게 세심히 맞춰주는 다정함이 돋보여요.',
 태약:'아이를 위해서라면 자기 것을 먼저 내어주는 헌신이 있어요.',
};
...
// 본문 첫 문장 50종 = 발달십성(5) × 일지십성(10).
// ※ 예전엔 발달십성 5종으로만 갈라서, 두 분의 발달십성이 같으면 첫 문장이 통째로 겹쳤음.
//   부모 카드가 복붙처럼 보이는 가장 큰 원인이라 일지 10종까지 곱해 50종으로 벌림.
const BODY10={
 비겁:{비견:'집에서도 위아래 없이 대등해서, 아이가 할 말은 하고 사는 공기가 흘러요.',
       겁재:'아이와 몸으로 부대끼며 놀다 금세 풀어지는, 씨름 같은 사이예요.',
       식신:'아이 눈높이에서 같이 웃어주니, 집이 편하고 시끌시끌해요.',
       상관:'아이 말에 곧장 받아치며 티격태격하는, 솔직한 공기가 흘러요.',
       정재:'나란히 서면서도 아이에게 필요한 건 빠짐없이 챙겨 놓아요.',
       편재:'함께 뛰어놀면서 필요한 건 주저 없이 크게 내주는 편이에요.',
       정관:'친구처럼 지내다가도 지킬 선 앞에서는 분명히 멈춰 세워요.',
       편관:'평소엔 편하게 대하다 중요한 순간엔 목소리가 단단해져요.',
       정인:'나란히 부딪히면서도 아이 속마음을 먼저 읽고 품어줘요.',
       편인:'같이 어울리는 와중에도 아이가 말하지 않은 것까지 알아채요.'},
 식상:{비견:'아이와 대등하게 떠들며, 눈치 볼 것 없는 집을 만들어요.',
       겁재:'온몸으로 놀아주다 같이 지쳐 뻗는, 에너지 넘치는 집이에요.',
       식신:'웃음이 끊이지 않아, 아이가 감정을 숨길 일이 없어요.',
       상관:'재치 있는 말로 아이를 웃기고, 아이도 덩달아 말이 늘어요.',
       정재:'신나게 놀아주면서도 준비물은 빠뜨리는 법이 없어요.',
       편재:'판을 크게 벌여 놀아주니, 집이 늘 놀이터처럼 들썩여요.',
       정관:'실컷 놀되 정리하고 자는 시간만큼은 지키게 해요.',
       편관:'마음껏 놀리다가도 위험한 선에선 단호하게 끊어요.',
       정인:'함께 놀며 아이 표정이 바뀌는 순간까지 읽어내요.',
       편인:'놀이 속에서 아이가 진짜 원하는 게 뭔지 짚어내요.'},
 재성:{비견:'필요한 걸 챙기면서도 아이와 나란히 뛰어주는 편이에요.',
       겁재:'챙길 건 챙기고, 같이 부딪히며 노는 것도 마다하지 않아요.',
       식신:'야무지게 챙기면서도 곁에선 편안하게 웃어줘요.',
       상관:'살뜰히 챙기며 재치 있는 말로 분위기를 풀어줘요.',
       정재:'챙기는 방식이 꼼꼼하고 일정해서, 아이가 예측 가능한 하루를 살아요.',
       편재:'필요하다 싶으면 주저 없이 크게 내줘, 아이가 부족함을 몰라요.',
       정관:'챙길 건 챙기고 지킬 선은 분명히 그어요.',
       편관:'다 챙겨주다가도 기준 앞에선 한 치도 물러서지 않아요.',
       정인:'필요한 걸 챙기면서 아이 마음까지 같이 살펴요.',
       편인:'말없이 챙겨놓고, 아이가 왜 그러는지 먼저 알아채요.'},
 관성:{비견:'기준은 세우되 아이와 눈높이를 맞춰 설명해요.',
       겁재:'원칙을 지키면서도 아이와 같이 뛰어주는 편이에요.',
       식신:'규칙 안에서는 마음껏 놀도록 풀어줘요.',
       상관:'틀은 잡되 아이의 엉뚱한 말은 살려주는 편이에요.',
       정재:'울타리를 세우고 그 안을 빠짐없이 챙겨요.',
       편재:'큰 틀만 정해두고 나머지는 넉넉히 맡겨요.',
       정관:'목욕·낮잠·식사 시간이 한결같아, 아이가 다음을 예상할 수 있어요.',
       편관:'기준이 분명해서, 아이가 어디까지가 선인지 또렷이 알아요.',
       정인:'기준은 지키되 아이의 속상함은 먼저 안아줘요.',
       편인:'원칙을 세우면서도 아이가 왜 못 지켰는지를 먼저 헤아려요.'},
 인성:{비견:'품어주면서도 아이를 한 사람으로 대해, 지시보다 의논이 먼저 나와요.',
       겁재:'깊이 품으면서도 아이와 부딪히는 걸 피하지 않아요.',
       식신:'품어주는 만큼 아이가 편하게 표현을 터뜨려요.',
       상관:'깊이 감싸주니 아이가 하고 싶은 말을 다 쏟아내요.',
       정재:'마음을 품으면서 필요한 것도 빠짐없이 챙겨요.',
       편재:'넉넉히 품고, 줄 수 있는 건 아낌없이 내주는 편이에요.',
       정관:'깊이 이해하되 지킬 선은 분명히 지켜요.',
       편관:'품어주다가도 필요한 순간엔 단단해지는 편이에요.',
       정인:'아이가 말하기 전에 먼저 다가가는, 늘 안심되는 자리예요.',
       편인:'겉말보다 그 뒤의 속내를 읽어내, 아이가 이해받는다고 느껴요.'},
};
// 3번째 축 — 발달십성(top)·일지십성(ss10)이 둘 다 같은 부부(약 3%)를 가르는 장치.
// EL_LEAD: 부모 일간 오행 → 헤드라인 앞머리 (카드에 이미 '금金/화火'로 표시되는 값이라 근거가 보임)
// SUB_OPEN: 부(2위) 십성 → 본문 첫 문장(단독형). 기존 SUB_TONE은 '여기에~'로 시작하는 뒷문장이라 따로 둠.
const EL_LEAD={ 목:'곧게 밀어주며', 화:'환하게 웃어주며', 토:'묵묵히 받쳐주며', 금:'반듯하게 챙기며', 수:'깊이 헤아리며' };
const SUB_OPEN={
 비겁:'아이를 아랫사람이 아니라 한 사람으로 대해, 지시보다 의논이 먼저 나와요.',
 식상:'잘 웃기고 잘 놀아줘서, 무거운 순간도 가볍게 풀어내는 재주가 있어요.',
 재성:'현실 감각이 야무져, 말뿐 아니라 실제로 손이 먼저 나가는 편이에요.',
 관성:'지킬 선을 아는 단단함이 있어, 다정하면서도 무르지 않은 균형을 지켜요.',
 인성:'아이 마음을 먼저 헤아려서, 다그치기 전에 한 번 더 들어줘요.',
};
function analyzeParent(pc){
  ...
  const brSS10=dzIdx>=0 ? tenGodEl(dayTG, CG_MAIN[dzIdx]) : '비견';   // 10종 원본
  const brSS=SS_CAT[brSS10]||'비겁';                                   // 5종 카테고리(분포·tie-break용)
  ...
  // 헤드라인 = 발달십성(5) × 일지십성(10) = 50종. 정재/편재가 같은 칸에 들어가 부모가 겹치던 문제 해결.
  const _core=(HEAD10[top]&&HEAD10[top][brSS10])||(HEAD_COMBO[top]&&HEAD_COMBO[top][brSS])||prof.head;
  // 일간 오행 앞머리를 붙여 50 → 250조합. top·일지가 둘 다 같은 부부도 여기서 갈림.
  const arche=`${EL_LEAD[pc.dayEl]||''} ${_core}`.replace(/\s+/g,' ').trim();
  // 사랑의 언어 = 발달십성 2개 + 일지십성 1개 → 같은 top이어도 일지가 다르면 갈림
  const _l2=String(prof.lang||'').split(' · ').slice(0,2).join(' · ');
  const _lb=LANG_BR10[brSS10]||LANG_BR[brSS];
  const trait=`사랑의 언어 — ${_l2}${_lb?' · '+_lb:''}`;
  // 본문 = [발달십성 정체성 1문장(유형 공통)] + [발달십성×일지 조합 1문장(25종·부모마다 갈림)]
  // ※ 예전엔 발달십성 정체성 문장(prof.body 1문장)을 맨 앞에 뒀는데, 두 분의 발달십성이 같으면
  //   본문이 '똑같은 문장'으로 시작해 복붙처럼 보였음. 게다가 그 문장은 헤드라인과 같은 말이라 중복.
  //   → 정체성 문장을 빼고, 조합별로 갈리는 문장을 첫 문장으로 올림.
  // 첫 문장 = 발달십성 × 일지십성 50종. 두 분의 발달십성이 같아도 일지가 다르면 반드시 갈림.
  const _b2=(BODY10[top]&&BODY10[top][brSS10]) || (COMBO_BODY[top]&&COMBO_BODY[top][brSS]) || BR_EMOTION[brSS] || '';
  // [축3] 부(2위) 십성 — top 다음으로 점수가 높은 기운. top·일지와 겹치면 생략해 중복 방지
  const _sub=Object.keys(dist).filter(k=>k!==top).sort((a,b)=>dist[b]-dist[a])[0];
  // 2번째 문장: 부십성이 top·일지와 겹치지 않으면 부십성으로, 겹치면 신강약 결로 (항상 뭔가는 붙게)
  const _b3=(_sub && dist[_sub]>0 && _sub!==brSS && _sub!==top)
    ? (SUB_TONE[_sub]||'') : (LEVEL_TONE[level]||'');
  // 본문 = [발달십성 × 일지 50종] + [부(2위)십성 5종]. 헤드라인은 일간 오행으로 또 갈리므로
  // top·일지가 둘 다 같은 부부라도 블록 전체가 같아질 확률은 매우 낮음.
  const _open=(_sub && dist[_sub]>0 && _sub!==top) ? (SUB_OPEN[_sub]||'') : (LEVEL_TONE[level]||'');
  const love=`${_b2} ${_open}`.replace(/\s+/g,' ').trim();
  ...
  return { dayPillar:TG[dayTG]+pc.dayBranch, dayEl:pc.dayEl, level, yong, hee, sipseongTop:top, sipseongSub:_sub, branchSipseong:brSS, branchSipseong10:brSS10,
    // GPT 프롬프트에 넘길 명리 원자값 (GPT가 직접 계산하지 않도록 판정은 전부 여기서)
    dayStem:TG[dayTG], dayBranch:pc.dayBranch, mulsang:P_MULSANG[dayTG]||'', strengthPct:+pr.toFixed(1), branchEl:ZHI_WX_[DZ.indexOf(pc.dayBranch)]||'', dist, arche, trait, love, watch, recharge };
}
...
// 안전망 — 발달십성·일지십성·부십성까지 전부 같은 부부(약 4%)는 본문이 통째로 겹친다.
// 신강약은 두 분이 다를 때가 많아 여기서 한 겹 더 갈라준다. (겹칠 때만 붙으므로 평소엔 문장이 길어지지 않음)
const LV_SPLIT={
 신강:'중심이 굳은 편이라, 한번 정한 방식은 좀처럼 흔들리지 않아요.',
 중화신강:'기운이 단단해서, 아이가 흔들릴 때 기준을 잡아주는 쪽이에요.',
 중화신약:'결이 섬세한 편이라, 아이 기분의 미묘한 변화를 남보다 빨리 알아채요.',
 신약:'마음이 여린 편이라, 아이 감정을 같이 느끼며 살펴요.',
 태약:'아이를 위해 애쓰다 먼저 지칠 수 있으니, 스스로를 챙기는 시간도 꼭 필요해요.',
};
...
function selectBirthDates({ mom, dad, dueFrom, dueTo }) {
  ...
  // 두 분 본문이 똑같아지면 신강약으로 한 겹 더 갈라줌 (엄마·아빠 카드가 복붙처럼 보이는 걸 막음)
  if(parentAn.length===2 && parentAn[0].love===parentAn[1].love){
    parentAn.forEach(a=>{ const t=LV_SPLIT[a.level]; if(t) a.love=(a.love+' '+t).trim(); });
  }
