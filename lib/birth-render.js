...
const { assignAxes } = require('./birth-axis.js');   // 구버전 payload 폴백용
...
function s01(p){
  ...
  const ps=src.map((a,i)=>`<div class="pcard ${i===1?'dad':''}"><div class="who">${a.who} <span class="pj">(${pillarHz(a.dayPillar)} 일주 · ${a.dayEl}${HZ[a.dayEl]}${a.sipseongTop?' · '+a.sipseongTop+' 발달':''}${a.branchSipseong10?' · 일지 '+a.branchSipseong10:''})</span></div>
...
// ── 선택 가이드 — 축은 엔진(birth-axis)이 GPT 호출 전에 배정한 것을 그대로 쓴다.
// 여기서 다시 계산하면 char_line(GPT)과 어긋나므로 절대 재계산하지 않는다.
function guideBox(dates){
  let picks=dates.map(f=>f.axis).filter(Boolean);
  // 구버전 리포트(payload에 axis 없음) 대비 — 그때만 즉석 배정
  if(picks.length!==dates.length) picks=assignAxes(dates);
  if(!picks || picks.filter(Boolean).length<2) return '';
  const rows=dates.map((f,i)=>{ const a=picks[i]; if(!a) return '';
    return `<div class="gdrow"><div class="gdtop"><div class="gdwish">${ic('heart')}<b>${esc(a.wish)}</b>를 바란다면</div>
      <div class="gdarrow">→</div><span class="gdd">${esc(f.date)}</span></div>
      <div class="gdtail">${esc(a.tail)}</div></div>`;
  }).join('');
...
function s03(p){
  ...
  return section('03','이 아이의 첫인상', body);
}
...
function s09(p){
  ...
  const _p0=(p.dates&&p.dates[0]&&p.dates[0].yeonun&&p.dates[0].yeonun.pts)||[];
  const _span=_p0.length ? `${_p0[0].year}~${_p0[_p0.length-1].year}` : '';
  return section('09','크게 피어나는 때 · 향후 10년 연운', body,
    `태어난 뒤 10년${_span?`(${_span})`:''}의 기운 흐름을 그래프로 담았어요. 아이가 아직 어린 시기라, 그 나이에 맞는 결과 곁에서 도와줄 점을 함께 짚었어요.`);
}
// 10 닫는 편지 · 3줄 요약
function s10(p){
  const body=(p.dates||[]).map(f=>{const c=f.content||{};
