...
function s07(p){
  ...
  const body=(p.dates||[]).map(f=>{const c=f.content||{};const cl=clingy(f,p.parents||[]);
    ...
    const fam = famRows
      ...
      : '';
    const clChip = cl.who==='both'
      ? `<span class="clchip">${ic('heart')}엄마·아빠 둘 다 껌딱지</span> <em>두 분 모두 이 아이에게 힘이 되는 <b>${cl.yong}(${HZ[cl.yong]}) 기운</b>을 지녔어요.</em>`
      : `<span class="clchip">${ic('heart')}${cl.who} 껌딱지</span> <em>${cl.who}가 이 아이에게 힘이 되는 <b>${cl.yong}(${HZ[cl.yong]}) 기운</b>을 일주에 더 든든히 지녔어요.</em>`;
    const head = fam ? `${fam}<div class="clline">${clChip}</div>` : `<div class="cling">${ic('heart')}<b>${cl.who==='both'?'엄마·아빠 둘 다 껌딱지':cl.who+' 껌딱지'}</b></div><div class="clline">${clChip}</div>`;
  ...
  const body=(p.dates||[]).map(f=>{const c=f.content||{};const cl=clingy(f,p.parents||[]);
    if(c.letter){
      ...
      return secBlock(f, `<div class="lbox"><div class="lq">“</div>${paras}
        <div class="lsign">— 아직 이름도 없는,<br><b>${esc(f.date)}의 아기가</b></div></div>`);
    }
...
.clline{font-size:12.5px;color:var(--ink2);line-height:1.7;margin:8px 0 2px}
.clchip{display:inline-block;background:#fff0e8;color:#c2604a;font-size:12px;font-weight:800;border-radius:999px;padding:3px 11px}
.clchip .ic{color:#c2604a}.clline em{font-style:normal}.clline b{color:var(--deep)}
