...
function ohaeng(f){
  ...
  return `${penta(f)}<div class="balance">${bars}</div>
...
function s01(p){
  ...
  const ps=src.map((a,i)=>`<div class="pcard ${i===1?'dad':''}"><div class="who">${a.who} <span class="pj">(${pillarHz(a.dayPillar)} 일주 · ${a.dayEl}${HZ[a.dayEl]}${a.sipseongTop?' · '+a.sipseongTop+' 발달':''})</span></div>
    <div class="ju">${esc(a.arche)}</div><p>${esc(a.love)}</p>${a.watch?`<div class="watch">${ic('eye')}${esc(a.watch)}</div>`:''}</div>`).join('');
  const img=`<div class="illust"><img src="${IL}/il-parents.png" alt="부모"></div>`;
  return section('01','우리는 어떤 부모가 될까?', img+ps,
...
function s02(p){
  ...
  const uniqBottom=f=>dates.length<2?[]:meta.filter(m=>{const v=f.eval.stages[m.key]; return dates.every(o=>o===f||o.eval.stages[m.key]>v);});
  ...
  const cmp=dates.map(f=>{
    ...
    // 후보 비교 — 셋 중 유일하게 높은/낮은 항목 (각 최대 2개)
    const leads=uniqTop(f).sort((a,b)=>f.eval.stages[b.key]-f.eval.stages[a.key]).slice(0,2).map(m=>m.label);
    const lags=uniqBottom(f).sort((a,b)=>f.eval.stages[a.key]-f.eval.stages[b.key]).slice(0,2).map(m=>m.label);
    const cmpLine = leads.length&&lags.length ? `<b>${leads.join('·')}</b>이 셋 중 가장 우수하고, <b>${lags.join('·')}</b>은 상대적으로 낮은 편이에요.`
      : leads.length ? `<b>${leads.join('·')}</b>이 셋 중 가장 우수해요.`
      : lags.length ? `튀는 최상위 항목은 없지만 <b>${lags.join('·')}</b> 정도만 상대적으로 낮고, 나머지는 상위권이에요.`
      : `특정 항목이 튀기보다 <b>전반적으로 고르게 안정적</b>인 후보예요.`;
    return `<div class="cmp col">
      ...
      ${radar(ev)}
      <div class="vs">${ic('scales')}<b>다른 후보와 비교하면</b> — ${cmpLine}</div>
...
function sparkBlock(f){
  ...
  const items=ss.map(s=>`<div class="sparkit"><div class="sph">${ic('spark')}<b>${s.name}</b> — ${esc(s.def)}</div><p>${esc(s.child)}</p><div class="tip">${ic('tip')}${esc(s.tip)}</div></div>`).join('');
  return `<div class="card sparkcard"><div class="bh2">${ic('star')}제 안의 특별한 반짝임</div>${items}</div>`;
}
...
function s03(p){
  ...
  const body=(p.dates||[]).map(f=>{const c=f.content||{};
    return `<div class="dwrap">${dHead(f)}
      ...
      <div class="card"><div class="dtype">${esc(c.type||'')}</div><p class="gist">${esc(c.gist||'')}</p>
...
function s06(p){
  const body=(p.dates||[]).map(f=>{const c=f.content||{};
    ...
    if((f.guiin||[]).length) inner+=`<p style="margin-top:10px">${ic('spark')}<b>곁을 지키는 귀인복</b> — <b>${f.guiin.join('·')}</b>이 있어 배움과 인복에서 귀인의 도움이 따르는 아이예요.</p>`;
...
function s07(p){
  ...
  const body=(p.dates||[]).map(f=>{const c=f.content||{};const cl=clingy(f,p.parents||[]);
    const rels=(c.bonds||[]).map(b=>`<div class="rel"><div class="relh">${b.who}와 <span class="reltg">${esc(b.tag)}</span></div><p>${esc(b.text)}</p></div>`).join('');
    ...
    const inyL=f.inyeon||[];
    const kLab=k=>k==='용신보충'?'꼭 필요한 기운':k==='생조'?'북돋는 별':'닮은 결';
    let iny='';
...
function s08(p){
  ...
  return section('08','부족한 기운을 채워주는 이름 · 작명 가이드', intro+body+foot,
...
function renderReport(p){
  ...
  const toc=`<section><div class="wrap"><div class="secno">목차</div><h2>이 책에 담긴 이야기</h2><ul class="toc">
    ...
    <li>우리 아이가 꽃피울 분야는</li><li>돈복 · 곁을 지키는 귀인</li><li>부모와의 관계 · 껌딱지</li><li>부족한 기운을 채워주는 이름</li>
...
.vs{font-size:13px;line-height:1.6;color:var(--ink2);background:#fdf6ee;border-radius:12px;padding:11px 14px;margin-top:8px;text-align:left}.vs b{color:var(--deep)}
/* 시주 곡선 */
...
.hbars{display:flex;align-items:flex-end;gap:3px;height:78px;padding:0 2px}
...
.hbnote{font-size:11.5px;color:var(--ink2);line-height:1.6;margin-top:9px}.hbnote b{color:var(--deep)}
/* 아기→부모 인연 */
