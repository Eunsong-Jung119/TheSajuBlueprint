...
function s01(p){
  ...
  const ps=src.map((a,i)=>`<div class="pcard ${i===1?'dad':''}"><div class="who">${a.who} <span class="pj">(${pillarHz(a.dayPillar)} 일주 · ${a.dayEl}${HZ[a.dayEl]}${a.sipseongTop?' · '+a.sipseongTop+' 발달':''})</span></div>
    <div class="ju">${esc(a.arche)}</div>${a.trait?`<p class="ptrait">${esc(a.trait)}</p>`:''}<p>${esc(a.love)}</p>${a.watch?`<div class="watch">${ic('eye')}${esc(a.watch)}</div>`:''}${a.recharge?`<div class="recharge">${ic('heart')}<b>${a.who}도 이럴 때 충전돼요</b> — ${esc(a.recharge)}</div>`:''}</div>`).join('');
...
const STAGE_BENEFIT={
  ...
  balance:'오행(목화토금수)이 고루 섞여 있어, 기질이 한쪽으로 치우치지 않고 무난하게 두루 어울리는 결이에요.',
  // ※ structure는 '합충형파해'를 재는 항목 — 충·형이 적고 합이 많을수록 높음. (집중력과는 무관)
  structure:'사주 글자들이 서로 부딪히는 충(沖)·형(刑) 없이 잘 어우러져 있어요. 갑작스러운 변동이나 큰 풍파에 휘둘리기보다, 잔잔하고 평탄하게 흘러가는 안정된 짜임이에요.',
  // ※ growth는 '십신의 갈래가 얼마나 다양한가'를 재는 항목 (학습력이 아니라 재능의 폭)
  growth:'타고난 재능의 갈래(십신)가 다양하게 갖춰져 있어요. 한 가지 길에만 매이지 않고, 커가면서 여러 방면에서 자기 몫을 찾아갈 수 있는 폭이 넓은 결이에요.',
...
// ── 선택 가이드 (결정론적) — "점수 1위"가 아니라 "부모의 바람"으로 고르게 ──
const _SSC10={비견:'비겁',겁재:'비겁',식신:'식상',상관:'식상',편재:'재성',정재:'재성',편관:'관성',정관:'관성',편인:'인성',정인:'인성'};
const _LVI=['태약','신약','중화신약','중화신강','신강'];
// 원국 6글자의 십성 카테고리 비중 (0~1)
function _ssMix(f){
  const m={비겁:0,식상:0,재성:0,관성:0,인성:0}; let n=0;
  (f.manseryeok||[]).forEach(c=>{
    [c.godGan,c.godZhi].forEach(g=>{ const k=_SSC10[g]; if(k){ m[k]++; n++; } });
  });
  const st=(f.eval&&f.eval.gyeok&&_SSC10[f.eval.gyeok.star]); if(st){ m[st]+=1.5; n+=1.5; } // 격국 가중
  if(!n) return m;
  Object.keys(m).forEach(k=>m[k]=m[k]/n);
  return m;
}
const _hasSin=(f,name)=>(f.sinsalChild||[]).some(s=>String(s.name||'').includes(name))?1:0;
// 부모가 흔히 품는 '바람' 7종 — 각각 원국 근거로 점수를 매겨 날짜에 하나씩 배정
const WISHES=[
 { k:'social', head:'사람들과 잘 어울리는 아이',
   tail:'낯선 자리에서도 먼저 말을 붙이고, 친구들 사이에서 기운을 얻어 오는 결이에요.',
   s:(f,m)=>m.식상*3+m.재성*2+_hasSin(f,'도화')*1.6+_hasSin(f,'역마')*1.1 },
 { k:'focus', head:'하나에 깊이 파고드는 아이',
   tail:'좋아하는 게 생기면 조용히 오래 붙잡고 늘어지는, 몰입으로 크는 결이에요.',
   s:(f,m)=>m.인성*3+_hasSin(f,'화개')*1.8+_hasSin(f,'문창')*1.5+_hasSin(f,'학당')*1.5 },
 { k:'lead', head:'앞에 나서서 이끄는 아이',
   tail:'무리 안에서 자연스럽게 앞자리에 서고, 정하면 미루지 않는 결이에요.',
   s:(f,m)=>m.비겁*3+_hasSin(f,'장성')*1.8+_hasSin(f,'양인')*1.4+_hasSin(f,'괴강')*1.4+(_LVI.indexOf(f.strengthLevel)>=3?0.8:0) },
 { k:'order', head:'반듯하고 믿음직한 아이',
   tail:'약속과 순서를 스스로 지켜서, 어른들 눈에 유난히 안심되는 결이에요.',
   s:(f,m)=>m.관성*3+((f.eval&&f.eval.stages&&f.eval.stages.structure)||0)/40 },
 { k:'creative', head:'자기만의 세계가 뚜렷한 아이',
   tail:'남이 안 보는 각도에서 이야기를 만들고, 제 방식대로 풀어내는 결이에요.',
   s:(f)=>{const st=(f.eval&&f.eval.gyeok&&f.eval.gyeok.star)||''; return (st==='상관'?3:0)+(st==='편인'?2.6:0)+_hasSin(f,'화개')*1.5+_hasSin(f,'홍염')*1.2;} },
 { k:'gentle', head:'다정하고 마음이 순한 아이',
   tail:'곁의 기분을 먼저 살피고, 부딪히기보다 품는 쪽을 고르는 결이에요.',
   s:(f,m)=>m.인성*1.6+(_LVI.indexOf(f.strengthLevel)<=1?1.8:0)+(((f.ohaeng||{}).토||0)+((f.ohaeng||{}).수||0))/40 },
 { k:'money', head:'현실 감각이 야무진 아이',
   tail:'제 것을 챙길 줄 알고, 손해 보지 않게 셈이 빠른 결이에요.',
   s:(f,m)=>m.재성*3+(f.wealthSinsal||[]).length*0.7 },
];
// 날짜 × 바람 점수표 → 그리디로 '한 날짜에 하나씩' 겹치지 않게 배정
// ※ 절대점수로 고르면 척도가 큰 바람(관성 등)만 계속 뽑힘 → 후보끼리의 '상대 우위'로 정규화해서 비교
function pickGuide(dates){
  if(!dates.length) return [];
  const raw=dates.map(f=>{ const m=_ssMix(f); return WISHES.map(w=>w.s(f,m)||0); });
  const cells=[];
  WISHES.forEach((_,wi)=>{
    const col=raw.map(r=>r[wi]);
    const mu=col.reduce((a,b)=>a+b,0)/col.length;
    const sd=Math.sqrt(col.reduce((a,b)=>a+(b-mu)*(b-mu),0)/col.length)||1;
    dates.forEach((_,di)=>cells.push({di,wi,v:(raw[di][wi]-mu)/sd + raw[di][wi]*0.01})); // 동점 시 절대값으로 미세 정렬
  });
  cells.sort((a,b)=>b.v-a.v);
  const outD=new Set(), outW=new Set(), res=[];
  for(const c of cells){ if(outD.has(c.di)||outW.has(c.wi)) continue;
    outD.add(c.di); outW.add(c.wi); res.push(c); if(res.length===dates.length) break; }
  // 혹시 못 채운 날짜가 있으면 남은 바람 중 아무거나
  dates.forEach((_,di)=>{ if(!outD.has(di)){ const wi=WISHES.findIndex((_,i)=>!outW.has(i));
    if(wi>=0){ outW.add(wi); outD.add(di); res.push({di,wi,v:0}); } } });
  return res.sort((a,b)=>a.di-b.di);
}
function guideBox(dates){
  const picks=pickGuide(dates); if(picks.length<2) return '';
  // ※ 설명문은 '바람'과 반드시 짝이 맞는 결정론적 문구를 씀(GPT가 쓴 char_line을 넣으면 서로 어긋날 수 있음)
  const rows=picks.map(({di,wi})=>{ const f=dates[di], w=WISHES[wi];
    return `<div class="gdrow"><div class="gdtop"><div class="gdwish">${ic('heart')}<b>${w.head}</b>를 바란다면</div>
      <div class="gdarrow">→</div><span class="gdd">${esc(f.date)}</span></div>
      <div class="gdtail">${w.tail}</div></div>`;
  }).join('');
  return `<div class="guidebox"><div class="gdh">${ic('compass')}<b>부모의 바람으로 고르는 선택 가이드</b></div>
    <div class="gdsub">점수는 ‘안정도’일 뿐이라, 1위가 곧 정답은 아니에요. 세 날짜는 <b>서로 다른 이야기</b>를 들려주니, 마음이 가는 쪽으로 고르셔도 좋아요.</div>
    ${rows}
    <div class="gdnote">※ “반드시 이렇게 태어난다”는 뜻이 아니라, <b>이 날짜가 들려주는 대표적인 이야기</b>예요. 아이는 그 위에 자기만의 결을 얹으며 자라요.</div></div>`;
}
...
function s02(p){
  ...
  const cmp=dates.map(f=>{
    ...
    return `<div class="cmp col">
      ...
      ${f.content?.char_line?`<div class="cmpchar">“${esc(f.content.char_line)}”</div>`:''}
  ...
  return section('02','왜 이 세 날짜가 남았을까요?', guideBox(dates)+frame+rangebox+cmp, '');
}
...
function s03(p){
  ...
  const body=(p.dates||[]).map(f=>{const c=f.content||{};
    return `<div class="dwrap">${dHead(f)}
      ...
      ${c.char_line?`<div class="charline"><span class="clq">한 문장으로 말하자면</span><b>${esc(c.char_line)}</b></div>`:''}
...
// 04 사용설명서 — 리포트의 주인공. 열리는 버튼 / 닫히는 순간 / 통하는 말 / 안 통하는 말 / 화났을 때
const _arr=v=>Array.isArray(v)?v.filter(x=>String(x||'').trim()):[];
function manualList(items, cls, mark){
  return `<ul class="mlist ${cls}">${items.map(x=>`<li>${mark?`<span class="mk">${mark}</span>`:''}<span>${esc(x)}</span></li>`).join('')}</ul>`;
}
// "대사 — 이유" 를 대사/이유로 쪼개 보여줌
function sayList(items, cls){
  return `<ul class="mlist ${cls}">${items.map(x=>{
    const s=String(x); const i=s.indexOf('—');
    const q=i>0?s.slice(0,i).trim():s.trim(), why=i>0?s.slice(i+1).trim():'';
    return `<li><q>${esc(q)}</q>${why?`<em>${esc(why)}</em>`:''}</li>`;
  }).join('')}</ul>`;
}
function s04(p){
  const body=(p.dates||[]).map(f=>{const c=f.content||{};
    const open=_arr(c.manual_open), close=_arr(c.manual_close);
    const say=_arr(c.manual_say), dont=_arr(c.manual_dont), angry=_arr(c.manual_angry);
    let inner=`${c.char_line?`<div class="mchar">${esc(c.char_line)}</div>`:`<div class="dtype2">${esc(c.type||'')}</div>`}
      <p class="mintro">${esc(c.manual_body)}</p>`;
    if(open.length||close.length){
      inner+=`<div class="mpair">
        ${open.length?`<div class="mbox mopen"><div class="mbh">${ic('heart')}<b>마음이 열리는 버튼</b></div>${manualList(open,'ok','＋')}</div>`:''}
        ${close.length?`<div class="mbox mclose"><div class="mbh">${ic('eye')}<b>마음이 닫히는 순간</b></div>${manualList(close,'ng','－')}</div>`:''}
      </div>`;
    }
    if(say.length||dont.length){
      inner+=`<div class="mpair">
        ${say.length?`<div class="mbox msay"><div class="mbh">${ic('check')}<b>잘 통하는 말</b></div>${sayList(say,'ok')}</div>`:''}
        ${dont.length?`<div class="mbox mdont"><div class="mbh">${ic('eye')}<b>잘 통하지 않는 말</b></div>${sayList(dont,'ng')}</div>`:''}
      </div>`;
    }
    if(angry.length){
      inner+=`<div class="angry"><div class="mbh">${ic('bulb')}<b>화가 났을 때 사용법</b></div>
        ${angry.map((x,i)=>`<div class="agrow"><span class="agn">${i+1}</span><div><em>${['지금 당장','조금 지나서','가라앉은 뒤'][i]||'그다음'}</em>${esc(x)}</div></div>`).join('')}</div>`;
    }
    inner+=`<div class="watch">${ic('eye')}<b>이런 점은 살펴주세요</b> — ${esc(c.manual_watch)}</div>
      <div class="tip">${ic('tip')}<b>부모님께</b> — ${esc(c.manual_tip)}</div>`;
    return secBlock(f, inner);
  }).join('');
  return section('04','우리 아이 사용설명서', body,
    '이 아이의 <b>마음이 열리는 순간과 닫히는 순간</b>, 통하는 말과 통하지 않는 말을 골라 담았어요. 리포트에서 가장 오래 꺼내 보게 될 장이에요.');
}
// 05 직업
function s05(p){
  const body=(p.dates||[]).map(f=>{const c=f.content||{};
    // ※ 표(td)로 두면 긴 근거 문구가 열 너비를 다 먹어 분야명이 한 글자씩 세로로 쪼개짐 → 세로 배치로
    const picks=(c.career_picks||[]).map(([a,b])=>{
      const neg=/^\s*[△▲✕xX]/.test(String(b));
      return `<div class="pkrow"><div class="pk">${esc(a)}</div><div class="pv ${neg?'no':'yes'}">${esc(b)}</div></div>`;
    }).join('');
    return secBlock(f, `<div class="ch">${esc(c.career_head||'')}</div><div class="pick">${picks}</div><p>${esc(c.career_desc)}</p>`);
  }).join('');
...
// 시트콤 장면 파싱 — "엄마: 대사" / "(지문)" 줄 단위
function sitcom(txt){
  const lines=String(txt||'').split(/\r?\n|(?<=[.」”』])\s{2,}/).map(s=>s.trim()).filter(Boolean);
  if(!lines.length) return '';
  const rows=lines.map(l=>{
    const m=l.match(/^(엄마|아빠|아이|아기|나)\s*[:：]\s*(.+)$/);
    if(m){ const who=m[1]; const cls=who==='엄마'?'mom':(who==='아빠'?'dad':'kid');
      return `<div class="scline ${cls}"><span class="scwho">${esc(who)}</span><span class="scsay">${esc(m[2])}</span></div>`; }
    return `<div class="scdir">${esc(l.replace(/^[（(]|[）)]$/g,''))}</div>`;
  }).join('');
  return `<div class="sitcom"><div class="sith">🎬 <b>우리 가족의 예상 시트콤</b></div>${rows}</div>`;
}
// 07 부모와 아이 — 가족 시트콤 + 명리 관계
function s07(p){
  ...
  const body=(p.dates||[]).map(f=>{const c=f.content||{};
    ...
    // 가족 시트콤 — 엄마와는 / 아빠와는 / 셋이 함께면 + 장면
    const famRows=[['엄마와는',c.family_mom],['아빠와는',c.family_dad],['셋이 함께 있으면',c.family_trio]]
      .filter(x=>x[1]).map(([k,v],i)=>`<div class="famrow f${i}"><b>${k}</b><span>${esc(v)}</span></div>`).join('');
    const head = famRows
      ? `<div class="fambox"><div class="famh">${ic('heart')}<b>우리 셋, 이렇게 굴러가요</b></div>${famRows}${sitcom(c.family_scene)}</div>`
      : '';
    ...
    return secBlock(f, `${head}<div class="dynh">${ic('compass')}<b>명리로 보면</b></div><div class="dyn">${rels}<div class="gem">${ic('spark')}<b>특별한 인연</b> — ${esc(c.gem)}</div></div>${inyBlock}`);
  }).join('');
  return section('07','우리 가족, 셋이 있으면 이런 그림', img+body,
    '엄마와 있을 때, 아빠와 있을 때, 셋이 함께일 때가 각각 어떻게 다른지 <b>한 장면처럼</b> 그려봤어요. 아래에는 그 근거가 되는 명리 관계도 함께 담았어요.');
}
...
function s10(p){
  const body=(p.dates||[]).map(f=>{const c=f.content||{};
    if(c.letter){
      // 줄바꿈이 없으면(대개 한 덩어리로 옴) 문장 2개씩 묶어 문단을 만들어 숨통을 틔움
      let chunks=String(c.letter).split(/\n+/).map(s=>s.trim()).filter(Boolean);
      if(chunks.length===1){
        const sents=chunks[0].split(/(?<=[.!?…])\s+/).map(s=>s.trim()).filter(Boolean);
        chunks=[]; for(let i=0;i<sents.length;i+=2) chunks.push(sents.slice(i,i+2).join(' '));
      }
      const paras=chunks.map(s=>`<p>${esc(s)}</p>`).join('');
      return secBlock(f, `<div class="lbox"><div class="lq">“</div>${paras}
        <div class="lsign">— <b>${esc(f.date)}의 아기가</b></div></div>`);
    }
    // 구버전 리포트(letter 없음) 대비 — 기존 3줄 요약 유지
    const lines=[
      `${esc(c.char_line||c.type||'')}`,
      `${ic('case')}${esc(c.career_head||'')}`,
      `${ic('heart')}<b>이 아이에게 힘이 되는 기운</b> — ${f.yongshin}(${HZ[f.yongshin]})`,
  ...
  const hasLetter=(p.dates||[]).some(f=>f.content&&f.content.letter);
  return section('10', hasLetter?'아기가 보내는 편지':'닫는 편지 · 3줄 요약', body+close,
    hasLetter?'첫 장에서 아기가 물었던 “어떤 결을 안고 태어날까요.” — 그 물음에 아기가 직접 답하는 편지예요. 날짜마다 결이 다르니, 편지도 세 통이에요.':'');
}
...
function renderReport(p){
  ...
  const toc=`<section><div class="wrap"><div class="secno">목차</div><h2>이 책에 담긴 이야기</h2><ul class="toc">
    <li>우리는 어떤 부모가 될까?</li><li>왜 이 3일일까 · 선택 가이드</li><li>이 아이의 첫인상</li><li>우리 아이 사용설명서</li>
    <li>우리 아이가 꽃피울 분야는</li><li>돈복 · 곁을 지키는 귀인</li><li>우리 가족 시트콤 · 부모와의 관계</li><li>균형을 돕는 기운을 담은 이름</li>
    <li>크게 피어나는 때 · 향후 10년</li><li>아기가 보내는 편지</li></ul></div></section>`;
  ...
  // ── 공유하기 (리포트 하단) ──
  const oid=String(p.orderId||'').replace(/[^a-zA-Z0-9]/g,'').slice(0,12);
  const share=`<section style="border:none"><div class="wrap"><div class="sharebox">
    <div class="shh">${ic('heart')}<b>이 스케치북, 함께 보고 싶은 사람이 있나요?</b></div>
    <div class="shsub">링크를 열면 이 리포트를 그대로 볼 수 있어요. 부모님 생년월일이 담겨 있으니 가까운 분들과만 나눠주세요.</div>
    <button type="button" class="shbtn primary" id="shReport">${ic('link')}리포트 공유하기</button>
    <button type="button" class="shbtn" id="shCopy">링크 복사</button>
    <div class="shdiv"></div>
    <div class="shsub2">우리 아이 이야기가 궁금한 친구가 있다면</div>
    <button type="button" class="shbtn ghost" id="shProduct">페이트랩 출산택일 추천하기</button>
    <div class="shtoast" id="shToast">링크를 복사했어요</div>
  </div></div></section>
  <script>(function(){
    var OID=${JSON.stringify(oid)};
    var PRODUCT='https://www.fatelab.co/birth?utm_source=share';
    function sid(){ try{ var s=sessionStorage.getItem('birth_sid');
      if(!s){ s='b'+Date.now().toString(36)+Math.random().toString(36).slice(2,8); sessionStorage.setItem('birth_sid',s); } return s;
    }catch(e){ return 'b'+Date.now().toString(36); } }
    function tb(name, extra){
      try{
        var body=JSON.stringify({ event_name:name, session_id:sid(), metadata:Object.assign({
          page:'report', order_id:OID,
          is_mobile:/Android|iPhone|iPad|iPod|Mobile/i.test(navigator.userAgent||'')
        }, extra||{}) });
        fetch('/api/track-birth',{ method:'POST', headers:{'Content-Type':'application/json'}, keepalive:true, body }).catch(function(){});
      }catch(e){}
    }
    tb('report_view');
    var toast=document.getElementById('shToast');
    function flash(msg){ if(!toast) return; toast.textContent=msg; toast.classList.add('on');
      setTimeout(function(){ toast.classList.remove('on'); }, 2200); }
    function copy(url, kind){
      function done(){ flash('링크를 복사했어요'); tb('report_share', {kind:kind, method:'copy'}); }
      if(navigator.clipboard && navigator.clipboard.writeText){
        navigator.clipboard.writeText(url).then(done).catch(function(){ prompt('아래 링크를 복사해 주세요', url); tb('report_share',{kind:kind,method:'prompt'}); });
      } else { prompt('아래 링크를 복사해 주세요', url); tb('report_share',{kind:kind,method:'prompt'}); }
    }
    function doShare(url, kind, title, text){
      tb('report_share_click', {kind:kind});
      if(navigator.share){
        navigator.share({ title:title, text:text, url:url })
          .then(function(){ tb('report_share', {kind:kind, method:'native'}); })
          .catch(function(err){ if(err && err.name==='AbortError') tb('report_share_cancel',{kind:kind}); else copy(url, kind); });
      } else { copy(url, kind); }
    }
    function myUrl(){ return location.origin + location.pathname; }   // 추적 파라미터 없는 깨끗한 링크로 공유
    var a=document.getElementById('shReport');
    if(a) a.addEventListener('click', function(){
      doShare(myUrl(), 'report', '우리 아기 사주 스케치북', '곧 만날 우리 아기의 타고난 결을 담은 스케치북이에요.'); });
    var b=document.getElementById('shCopy');
    if(b) b.addEventListener('click', function(){ tb('report_share_click',{kind:'report'}); copy(myUrl(), 'report'); });
    var c=document.getElementById('shProduct');
    if(c) c.addEventListener('click', function(){
      doShare(PRODUCT, 'product', '페이트랩 출산택일 · 우리 아기 사주 스케치북', '곧 만날 아기의 타고난 결을 그려주는 출산택일 리포트예요.'); });
  })();<\/script>`;
...
.pkrow{padding:10px 2px;border-bottom:1px solid #f2ece3}.pkrow:last-child{border-bottom:none}
.pk{font-size:14.5px;font-weight:800;color:var(--deep);word-break:keep-all}
.pv{font-size:13px;line-height:1.6;margin-top:3px;word-break:keep-all}
.pv.yes{color:#3f8f5f}.pv.no{color:var(--ink2)}
...
.recharge{background:#fdf4f6;border-radius:12px;padding:12px 14px;font-size:13.5px;line-height:1.7;margin-top:8px;color:var(--ink)}.recharge b{color:var(--deep)}
/* 섹션1 두 분 공통점 */
...
/* ── 선택 가이드 (02) ── */
.guidebox{background:linear-gradient(180deg,#fff6ec,#fdf2f6);border:1px solid #f0dccb;border-radius:18px;padding:16px 16px 14px;margin-bottom:14px}
.gdh{font-size:15px;color:var(--deep);margin-bottom:4px}.gdh b{font-weight:800}
.gdsub{font-size:12.5px;color:var(--ink2);line-height:1.65;margin-bottom:10px}.gdsub b{color:var(--deep)}
.gdrow{background:#fff;border-radius:13px;padding:11px 13px;margin:7px 0;box-shadow:0 3px 10px rgba(160,140,120,.06)}
.gdtop{display:flex;align-items:center;gap:8px}
.gdwish{flex:1;font-size:13px;color:var(--ink2);line-height:1.5;word-break:keep-all}.gdwish b{color:var(--deep);font-weight:800;font-size:14px}
.gdarrow{flex:0 0 auto;color:var(--peach);font-weight:900;font-size:15px}
.gdd{flex:0 0 auto;font-size:18px;font-weight:900;color:var(--deep);line-height:1.2}
.gdtail{font-size:12px;color:var(--ink2);line-height:1.6;margin-top:5px;padding-top:6px;border-top:1px dashed #f2e8dc;word-break:keep-all}
.gdnote{font-size:11.5px;color:var(--ink2);line-height:1.6;margin-top:10px}.gdnote b{color:var(--deep)}
.cmpchar{font-size:15px;font-weight:800;color:var(--deep);line-height:1.5;margin:6px 0 2px;word-break:keep-all}
/* ── 캐릭터 한 줄 (03) ── */
.charline{background:linear-gradient(135deg,#fff3e8,#fbeef4);border:1px solid #f2ddcb;border-radius:16px;padding:14px 16px;margin:10px 0 2px;text-align:center}
.clq{display:block;font-size:11px;font-weight:800;color:var(--peach);letter-spacing:.04em;margin-bottom:4px}
.charline b{font-size:19px;font-weight:900;color:var(--deep);line-height:1.45;word-break:keep-all}
/* ── 사용설명서 (04) ── */
.mchar{font-size:17px;font-weight:900;color:var(--deep);line-height:1.45;margin-bottom:4px;word-break:keep-all}
.mintro{font-size:14px;line-height:1.75;color:var(--ink)}
.mpair{display:grid;grid-template-columns:1fr;gap:8px;margin-top:10px}
@media(min-width:430px){.mpair{grid-template-columns:1fr 1fr}}
.mbox{border-radius:14px;padding:12px 13px}
.mbox.mopen{background:#f2f8f3;border:1px solid #dcebe0}
.mbox.mclose{background:#fbf4f2;border:1px solid #f0dfd9}
.mbox.msay{background:#f3f7fb;border:1px solid #dde8f0}
.mbox.mdont{background:#faf6ee;border:1px solid #eee2cd}
.mbh{font-size:13px;color:var(--deep);margin-bottom:6px;display:flex;align-items:center}.mbh b{font-weight:800}
.mbox.mopen .mbh .ic{color:#4f9a6a}.mbox.mclose .mbh .ic{color:#c98a76}.mbox.msay .mbh .ic{color:#5b8fb9}.mbox.mdont .mbh .ic{color:#c0993f}
.mlist{list-style:none;margin:0;padding:0}
.mlist li{display:flex;gap:5px;font-size:13px;line-height:1.65;color:var(--ink);padding:5px 0;border-top:1px dashed rgba(0,0,0,.06);word-break:keep-all}
.mlist li:first-child{border-top:none}
.mlist li>*{min-width:0}
.mlist .mk{flex:0 0 12px;font-weight:900;color:#8aa;text-align:center}
.mlist.ok .mk{color:#4f9a6a}.mlist.ng .mk{color:#c98a76}
.mlist q{display:block;font-size:13.5px;font-weight:700;color:var(--deep);quotes:'“' '”'}
.mlist q:before{content:open-quote}.mlist q:after{content:close-quote}
.mlist em{display:block;font-style:normal;font-size:11.5px;color:var(--ink2);line-height:1.55;margin-top:2px}
.angry{background:#fdf3f4;border:1px solid #f2dfe0;border-radius:14px;padding:12px 13px;margin-top:10px}
.angry .mbh .ic{color:#c2604a}
.agrow{display:flex;gap:9px;align-items:flex-start;margin:7px 0}
.agn{flex:0 0 20px;height:20px;border-radius:50%;background:#e79a86;color:#fff;font-size:11.5px;font-weight:800;display:flex;align-items:center;justify-content:center;margin-top:2px}
.agrow>div{flex:1;font-size:13px;line-height:1.65;color:var(--ink);word-break:keep-all}
.agrow em{display:block;font-style:normal;font-size:10.5px;font-weight:800;color:var(--peach);margin-bottom:1px}
/* ── 가족 시트콤 (07) ── */
.fambox{background:linear-gradient(180deg,#fff7ef,#fdf2f6);border:1px solid #f1e0d2;border-radius:16px;padding:14px 15px;margin-bottom:10px}
.famh{font-size:14.5px;color:var(--deep);margin-bottom:8px}.famh b{font-weight:800}
.famrow{background:#fff;border-radius:12px;padding:10px 12px;margin:6px 0;font-size:13.5px;line-height:1.65;color:var(--ink)}
.famrow b{display:block;font-size:12px;font-weight:800;color:var(--peach);margin-bottom:2px}
.famrow.f2 b{color:var(--gold)}
.sitcom{background:#fffdf9;border:1px dashed #e6d3c0;border-radius:14px;padding:12px 13px;margin-top:10px}
.sith{font-size:13px;color:var(--ink2);margin-bottom:8px}.sith b{color:var(--deep);font-weight:800}
.scdir{font-size:12px;color:var(--ink2);font-style:italic;line-height:1.6;margin:5px 0;word-break:keep-all}
.scline{display:flex;gap:8px;align-items:flex-start;margin:6px 0}
.scwho{flex:0 0 auto;font-size:11px;font-weight:800;color:#fff;background:var(--lav);border-radius:999px;padding:2px 9px;margin-top:2px}
.scline.dad .scwho{background:var(--blue)}.scline.kid .scwho{background:var(--peach)}
.scsay{flex:1;font-size:13.5px;line-height:1.65;color:var(--ink);word-break:keep-all}
.dynh{font-size:12px;color:var(--ink2);margin:12px 0 -2px}.dynh b{color:var(--deep);font-weight:800}
/* ── 아기가 보내는 편지 (10) ── */
.lbox{background:linear-gradient(180deg,#fffaf4,#fdf3f7);border:1px solid #f1e2d4;border-radius:16px;padding:18px 18px 16px;position:relative}
.lq{font-size:42px;line-height:.6;color:#f0cdb6;font-weight:900;margin-bottom:6px}
.lbox p{font-size:14.5px;line-height:1.95;color:var(--ink);margin:0 0 6px}
.lsign{text-align:right;font-size:12.5px;color:var(--ink2);line-height:1.7;margin-top:12px}.lsign b{color:var(--deep);font-weight:800;font-size:13.5px}
/* ── 공유하기 ── */
.sharebox{background:linear-gradient(180deg,#fff6ec,#fdf2f6);border:1px solid #f0dccb;border-radius:18px;padding:18px 16px;text-align:center;position:relative}
.shh{font-size:15px;color:var(--deep);margin-bottom:4px}.shh b{font-weight:800}
.shsub{font-size:12px;color:var(--ink2);line-height:1.65;margin-bottom:12px;word-break:keep-all}
.shbtn{display:block;width:100%;border:1px solid #eddcc9;background:#fff;color:var(--deep);font-family:inherit;font-size:14.5px;font-weight:800;border-radius:12px;padding:13px 14px;margin:7px 0;cursor:pointer;-webkit-tap-highlight-color:transparent}
.shbtn:active{transform:translateY(1px)}
.shbtn .ic{color:inherit}
.shbtn.primary{background:var(--peach);border-color:var(--peach);color:#fff;box-shadow:0 4px 14px rgba(231,154,134,.35)}
.shbtn.ghost{background:transparent;border-style:dashed;color:var(--ink2);font-size:13.5px;font-weight:700}
.shdiv{height:1px;background:#f0e2d4;margin:16px 0 12px}
.shsub2{font-size:12px;color:var(--ink2);margin-bottom:2px}
.shtoast{position:fixed;left:50%;bottom:28px;z-index:50;transform:translate(-50%,10px);background:rgba(64,50,59,.94);color:#fff;font-size:13px;font-weight:700;border-radius:999px;padding:10px 20px;opacity:0;pointer-events:none;transition:opacity .2s,transform .2s;white-space:nowrap;box-shadow:0 6px 20px rgba(0,0,0,.18)}
.shtoast.on{opacity:1;transform:translate(-50%,0)}
</style></head><body>
${cover}${letter}${toc}
