...
function renderReport(p){
  ...
  const share=`<section style="border:none"><div class="wrap"><div class="sharebox">
    ...
    <div class="shtoast" id="shToast">링크가 복사되었습니다</div>
  </div></div></section>
  <script>(function(){
    ...
    // 클립보드 미지원 환경(구형 사파리 등) 폴백 — 화면 밖 input에 넣고 execCommand
    function legacyCopy(url){
      try{
        var t=document.createElement('input'); t.value=url;
        t.setAttribute('readonly',''); t.style.cssText='position:fixed;top:-9999px;opacity:0';
    ...
    function copy(url, kind){
      ...
      function done(){ flash('링크가 복사되었습니다'); tb('report_share', {kind:kind, method:'copy'}); }
      function fail(){
        if(legacyCopy(url)){ flash('링크가 복사되었습니다'); tb('report_share',{kind:kind, method:'legacy'}); }
    ...
    if(a) a.addEventListener('click', function(){ copy(myUrl(), 'report'); });
    var c=document.getElementById('shProduct');
    if(c) c.addEventListener('click', function(){ copy(PRODUCT, 'product'); });
  })();<\/script>`;
