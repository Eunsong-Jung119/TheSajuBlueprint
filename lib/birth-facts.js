...
const NAME_POOL={
  여아:[['가은','인기'],['규리',''],['가율',''],['고은',''],
       ['나은',''],['다은','인기'],['라온',''],['도아',''],['리아',''],
       ['하윤','인기'],['하은','인기'],['예은',''],['아윤',''],
       ['서아','인기'],['서연','인기'],['지우','인기'],['지안','인기'],['채원',''],['지민','인기'],
       ['민서','인기'],['하민','인기'],['유민',''],['보민','']],
  남아:[['건우','인기'],['규현','인기'],['가온',''],['규민',''],
       ['도현','인기'],['도윤','인기'],['로운',''],['태오',''],
       ['하준','인기'],['현우','인기'],['은우',''],['유준',''],
       ['서준','인기'],['시우','인기'],['준우','인기'],['지호','인기'],['지훈',''],
       ['민준','인기'],['민재',''],['범준',''],['보윤','']] };
function naming(gender, yong, hee, gi){
  ...
  const scored=pool.map(([n,p])=>{
    ...
    return { n, p, hit:els.filter(e=>want.has(e)).length, bad:els.filter(e=>e===gi).length };
  });
  // 용신·희신 소리를 실제로 담은 이름만: 매칭 많은 순 → 기신 적은 순 → 인기 순
  const fit=scored.filter(s=>s.hit>0)
    .sort((a,b)=> (b.hit-a.hit) || (a.bad-b.bad) || ((b.p?1:0)-(a.p?1:0)));
  ...
  if(list.length<3){
    ...
    const extra=scored.filter(s=>s.hit===0 && !have.has(s.n))
