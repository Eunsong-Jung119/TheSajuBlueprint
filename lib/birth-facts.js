...
const SS_STANCE10={
 비견:'친구처럼 대등하게 지내는 사이예요. 위아래 없이 함께 부딪히고 편하게 어울려요.',
 겁재:'든든한 동지 같은 사이예요. 서로 밀어주고 끌어주며 함께 크는 관계예요.',
 식신:'{부모} 앞에서 재롱과 표현이 마음껏 터지는, 아이가 편하게 끼를 펼치는 사이예요.',
 상관:'{부모}에게 반짝이는 재치와 아이디어를 마음껏 쏟는, 표현이 톡톡 튀는 사이예요.',
 편재:'아이가 {부모}에게 스스럼없이 다가가 활달하게 어울리는, 밝고 시원시원한 관계예요. 격식 없이 편하게 장난도 치고 먼저 표현하는, 통통 튀는 사이예요.',
 정재:'아이가 {부모}를 살뜰히 아끼고 챙기는, 서로를 소중히 여기는 다정하고 안정적인 관계예요.',
 편관:'{부모}가 씩씩한 울타리가 되어, 아이가 그 안에서 긴장과 안정을 함께 배우는 사이예요.',
 정관:'{부모}가 반듯한 기준이 되어, 아이가 그 안에서 질서와 안정감을 얻는 믿음직한 사이예요.',
 편인:'{부모}가 아이의 속마음을 깊이 읽고 품어주는, 남다른 이해로 감싸는 사이예요.',
 정인:'{부모}가 아이를 헌신적으로 품고 먹여주는, 사랑을 흠뻑 받는 따뜻한 사이예요.',
};
const REL_NUANCE_B={
...
function parentBond(bf,parent,who){
  ...
  // 2번째 문장: 부모 양육유형(top 십성)별 서술 우선 → 같은 천간이어도 엄마/아빠가 갈림. 없으면 일지관계로 폴백.
  const tail = (parent.sipseongTop && PARENT_RECEIVE[parent.sipseongTop]) || REL_NUANCE_B[br] || '';
  // 3번째 문장: 부딪힐 수 있는 점 + 화해 열쇠 (관계를 입체적으로)
  const clash = SS_CLASH[SSCAT10[ss]] || '';
  const text=`${(SS_STANCE10[ss]||'').replace(/\{부모\}/g,who)} ${tail.replace(/\{부모\}/g,who)} ${clash.replace(/\{부모\}/g,who)}`.trim();
...
function buildFacts(selection, gender){
  ...
  return selection.top3.map(c=>{
    ...
    const bonds=(pAn.length?pAn:selection.parents.map((p,i)=>({who:i===0?'엄마':'아빠',...p}))).map(pa=>({who:pa.who, ...parentBond(f,pa,pa.who)}));
