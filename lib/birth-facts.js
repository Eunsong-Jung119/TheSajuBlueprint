...
const NAME_POOL={ 여아:[['지민','인기'],['민서','인기'],['하민','인기'],['가은','인기'],['유민',''],['규리',''],['가율',''],['보민','']],
  남아:[['민준','인기'],['건우','인기'],['규현','인기'],['가온',''],['규민',''],['보윤',''],['민재',''],['가람','']] };
function naming(gender, yong, hee){
  const want=new Set([yong,hee]);
  const pool=NAME_POOL[gender==='남아'?'남아':'여아'];
  const fit=pool.filter(([n])=> n.split('').some(ch=>want.has(choWx(ch))));
  const rest=pool.filter(x=>!fit.includes(x));
  const list=[...fit,...rest].slice(0,6).map(([n,p])=>({name:n, tag:nameTag(n), pop:!!p}));
