...
const SINSAL_CHILD={
 ...
 도화:{name:'도화살',def:'사람을 끌어당기는 매력의 별. 눈길을 모으는 힘이에요.',child:'표정이 사랑스럽고 사람을 웃게 하는 재주가 있어요. 평소엔 조용하다가 좋아하는 자리에선 애교가 톡 터지는 결이에요.',tip:'끼를 누르기보다 함께 즐거워해 주면 자존감이 예쁘게 자라요.'},
 ...
 문창:{name:'문창귀인',def:'글·공부와 인연이 깊은 대표 학문 귀인성이에요.',child:'새로운 걸 익히고 궁금한 걸 파고드는 총명함이 있어요. 책·이야기 속 세계에 잘 빠지는 아이예요.',tip:'정답을 바로 알려주기보다 “같이 찾아볼까?”로 호기심을 이어주면 좋아요.'},
 학당:{name:'학당귀인',def:'배움터의 별 — 학습·집중과 통하는 귀인성이에요.',child:'배우는 자리에서 유독 반짝여요. 한 가지를 꾸준히 익히는 끈기가 강점이에요.',tip:'결과보다 “오늘 새로 알게 된 것”을 물어봐 주면 배움이 즐거워져요.'},
 괴강:{name:'괴강살',def:'흔치 않은 카리스마·결단의 별이에요.',child:'주관이 또렷하고, 한번 마음먹으면 끝까지 밀고 나가는 뚝심이 있어요. “내가 할래”가 강한 야무진 아이예요.',tip:'고집으로 보일 때도 이유를 들어주면 그 뚝심이 리더십으로 자라요.'},
 양인:{name:'양인살',def:'날카로운 집중과 승부욕의 별이에요.',child:'좋아하는 일엔 무섭게 파고들고, 실력을 갈고닦는 전문가 기질이에요.',tip:'승패보다 “어제보다 나아진 것”을 칭찬해 주면 집념이 건강하게 커요.'},
 장성:{name:'장성살',def:'무리의 중심에 서는 통솔의 별이에요.',child:'친구들 사이에서 자연스럽게 중심이 되고, 상황을 정리하고 이끄는 리더 기질이 있어요.',tip:'“대장 노릇”을 나무라기보다 친구 마음도 챙기는 리더가 되게 도와주면 좋아요.'},
 홍염:{name:'홍염살',def:'은근한 분위기 매력의 별이에요.',child:'요란하지 않아도 곁에 있으면 눈이 가는, 사람을 편하게 하는 매력을 지녔어요.',tip:'인기보다 “사람을 편하게 해주는 점”을 짚어주면 매력이 내면까지 예뻐져요.'},
};
...
function parentBond(bf,parent,who){
  ...
  const text=`${(SS_STANCE10[ss]||'').replace(/\{부모\}/g,who)} ${tail.replace(/\{부모\}/g,who)}`.trim();
