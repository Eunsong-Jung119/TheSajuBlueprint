...
function getTenGod(dayTG, targetTG) {
  ...
  if (ctrlBy[dE]===tE) return tY!==dY ? 'Institutional Authority (정관)' : 'Pressure Authority (편관)';
...
function calcSipseongV3(pillars, dayTG) {
  ...
  const EN_TO_KR = {
    ...
    'Institutional Authority (정관)': '정관',
    'Pressure Authority (편관)': '편관',
...
// ============================================================
// calcStrengthScore v2
// 위치 가중치 기반 신강/신약 계산
// 기존 함수를 이걸로 교체 (saju-engine.js 내 calcStrengthScore)
// ============================================================
//
// 위치별 가중치:
//   월지(Month Branch): 35점
...
//   일간(나)는 기준점이므로 제외
//
// Supporters(인성+비겁): 나를 돕는 오행 자리의 점수 합산
// Drainers(식상+재성+관성): 내 힘을 빼는 오행 자리 점수 합산
// 50점 이상 → 신강(Strong), 50점 미만 → 신약(Weak)
//
// 지장간(본기)도 해당 자리 점수의 0.5배로 반영
// ============================================================

function calcStrengthScore(pillars, dayTG) {
  const TG_ELEM  = ['Wood','Wood','Fire','Fire','Earth','Earth','Metal','Metal','Water','Water'];
  const DZ_ELEM  = ['Water','Earth','Wood','Wood','Earth','Fire','Fire','Earth','Metal','Metal','Earth','Water'];
  const DZ_MAIN_TG = [9,5,0,1,4,2,3,5,6,7,4,8]; // 지장간 본기

  const dayElem = TG_ELEM[dayTG];

  // 일간을 돕는 오행 (인성: 나를 생하는 오행)
  const genBy = { Fire:'Wood', Earth:'Fire', Metal:'Earth', Water:'Metal', Wood:'Water' };
  const supportElem = genBy[dayElem]; // 인성 오행

  // 일간과 같은 오행 = 비겁
  // supportElem = 인성

  function isSupporter(elem) {
    return elem === dayElem || elem === supportElem;
  }

  // 위치별 가중치
  // pillars = [년주(0), 월주(1), 일주(2), 시주(3)]
  const BRANCH_WEIGHT = { 0: 8, 1: 35, 2: 18, 3: 12 }; // 지지
  const STEM_WEIGHT   = { 0: 10, 1: 10, 3: 10 };         // 천간 (일간=2 제외)

  let supportScore = 0;  // Supporters 합산
  let drainScore   = 0;  // Drainers 합산
  const breakdown  = [];

  pillars.forEach(([tg, dz], idx) => {
    // ── 천간 (일간 제외)
    if (idx !== 2) {
      const tgElem = TG_ELEM[tg];
      const w = STEM_WEIGHT[idx] || 10;
      if (isSupporter(tgElem)) {
        supportScore += w;
        breakdown.push(`Stem[${idx}] ${tgElem} +${w} (supporter)`);
      } else {
        drainScore += w;
        breakdown.push(`Stem[${idx}] ${tgElem} +${w} (drainer)`);
      }
    }

    // ── 지지
    const dzElem = DZ_ELEM[dz];
    const branchW = BRANCH_WEIGHT[idx] || 8;
    if (isSupporter(dzElem)) {
      supportScore += branchW;
      breakdown.push(`Branch[${idx}] ${dzElem} +${branchW} (supporter)`);
    } else {
      drainScore += branchW;
      breakdown.push(`Branch[${idx}] ${dzElem} +${branchW} (drainer)`);
    }

    // ── 지장간 본기 (해당 지지 가중치의 0.5배)
    const mainTGIdx = DZ_MAIN_TG[dz];
    if (mainTGIdx >= 0) {
      const mainElem = TG_ELEM[mainTGIdx];
      const hiddenW  = Math.round(branchW * 0.5);
      if (isSupporter(mainElem)) {
        supportScore += hiddenW;
        breakdown.push(`Hidden[${idx}] ${mainElem} +${hiddenW} (supporter)`);
      } else {
        drainScore += hiddenW;
        breakdown.push(`Hidden[${idx}] ${mainElem} +${hiddenW} (drainer)`);
      }
    }
  });

  const total = supportScore;
  // 50점 이상 = 신강, 미만 = 신약
  // 추가로 점수 구간별 세분화
  let label, isStrong;
  if      (total >= 70) { label = 'Very Strong (강왕)'; isStrong = true;  }
  else if (total >= 55) { label = 'Strong (신강)';      isStrong = true;  }
  else if (total >= 45) { label = 'Balanced (중화)';    isStrong = false; }
  else if (total >= 30) { label = 'Weak (신약)';        isStrong = false; }
  else                  { label = 'Very Weak (극약)';   isStrong = false; }

  return {
    total,          // supporter 점수 (0~100+)
    drainTotal: drainScore,
    label,
    isStrong,
    // 디버그용
...
//   strength.total  → supporter 점수 (50 이상이면 신강)
//   strength.label  → "Strong (신강)" 등
//   strength.isStrong → boolean
//
// short-report.js에서 strengthLabel 뽑는 부분:
//   const strength = calcStrengthScore(saju.pillars, saju.dayTG);
//   // strength.label 바로 사용
//   // strength.total로 점수 확인 가능
//
// ctx에 점수 추가하려면 buildReportContext에서:
//   strengthScore: strength.total,  // 추가
// ============================================================
...
function buildDaewonContext(daewonResult, dayTG, yongshinElem, gishinElem, currentYear=2026) {
  ...
  const GOD_THEMES = {
    ...
    'Institutional Authority (정관)':        'career recognition, institutional advancement, commitment energy, structured success',
    'Pressure Authority (편관)':          'transformative pressure, authority challenges, high-risk high-reward power moves',
...
function calcRelationalStructure(pillars, dayTG, strengthResult, sipseongData, hapchung) {
  ...
  // ── pickRedFlag: deterministic 선택 (intimacyStyle → relationshipRisk → dom1 → clash 순위)

  // ════════════════════════════════════════════════════════
  // RELATIONSHIP ARCHETYPE SYSTEM (deterministic, score-based)
  // 16 Love Nature × 20 Red Flag × 16 Heartbreak × 16 Loves You Right
  // ════════════════════════════════════════════════════════

  // ── Relationship Signal Scores — dom1 기반으로 dominant signal 결정
  const relSignals = {
    independence:  Math.min(100, Math.round(normP * 0.7 + (dom1==='P' ? 20 : 0))),
    emotionalDepth: Math.min(100, Math.round(emotionalDepth * (dom1==='R' ? 1.1 : 0.8))),
    stability:     Math.min(100, Math.round(normA * 0.55 + normW * 0.2 + (hasCombine ? 15 : 0) + (dom1==='A' ? 15 : 0))),
    intensity:     Math.min(100, Math.round(normO * 0.6 + (hasClash ? 20 : 0) + (sangGwan > 0.6 ? 15 : 0) + (dom1==='O' ? 15 : 0))),
    idealism:      Math.min(100, Math.round(normR * 0.65 + (dom1==='R' ? 20 : 0) + (isShinYak && dom1==='R' ? 10 : 0))),
  };

  // ── Love Nature selection
  const LN_TEMPLATES = {
    LN1:  { key: 'Independent Explorer',    body: 'You value emotional connection but also need personal freedom to feel balanced. Relationships work best when both partners maintain individuality while supporting each other\'s growth. You tend to thrive with people who respect autonomy rather than control it. When this balance exists, your relationships feel energizing rather than restrictive.' },
    LN2:  { key: 'Deep Emotional Connector', body: 'Emotional depth is the center of how you experience relationships. You naturally seek partners who are open, introspective, and capable of meaningful conversation. Once trust forms, your loyalty and care run extremely deep. Relationships for you become powerful emotional partnerships rather than casual connections.' },
    LN3:  { key: 'Loyal Builder',           body: 'You approach relationships with a strong sense of commitment and reliability. Stability and long-term trust matter more to you than temporary excitement. Once you invest emotionally, you show up consistently and with dedication. The strongest relationships for you are built slowly on shared direction and mutual support.' },
    LN4:  { key: 'Expressive Romantic',     body: 'Emotion and expression flow naturally in your relationships. You enjoy sharing feelings, experiences, and ideas openly with your partner. This openness creates vibrant emotional connections. When matched with someone who appreciates this honesty, relationships become deeply fulfilling.' },
    LN5:  { key: 'Thoughtful Observer',     body: 'You approach relationships with reflection and emotional awareness. Rather than rushing into connections, you take time to understand someone\'s character. This thoughtful approach helps you build meaningful and stable bonds. Once trust is established, your loyalty becomes unwavering.' },
    LN6:  { key: 'Intuitive Empath',        body: 'You naturally sense the emotions and needs of people around you. Relationships for you are built through emotional understanding and quiet support. Because you are highly intuitive, you often notice feelings others struggle to express. When your empathy is appreciated, relationships become deeply harmonious.' },
    LN7:  { key: 'Adventurous Partner',     body: 'You experience relationships as a shared journey of discovery. Excitement, growth, and new experiences keep your emotional world alive. Partners who embrace curiosity and exploration tend to energize you the most. With the right person, relationships become an exciting adventure.' },
    LN8:  { key: 'Strategic Companion',     body: 'You tend to view relationships through a thoughtful and pragmatic lens. Compatibility, shared values, and long-term direction matter greatly to you. Rather than impulsive romance, you prefer connections built on mutual goals and understanding. This creates partnerships that are both stable and purposeful.' },
    LN9:  { key: 'Gentle Supporter',        body: 'You express love through quiet care and emotional presence. Small gestures and consistent support mean more to you than dramatic displays. Partners often feel safe and understood in your presence. Over time this creates deeply nurturing relationships.' },
    LN10: { key: 'Curious Communicator',    body: 'Communication plays a central role in your relationships. You enjoy exploring ideas, emotions, and perspectives together with a partner. Intellectual and emotional curiosity help you build strong bonds. The best relationships for you feel like ongoing conversations.' },
    LN11: { key: 'Protective Partner',      body: 'You naturally take responsibility for the well-being of people you care about. Loyalty and dependability define your approach to relationships. Partners often feel protected and supported by your presence. When balanced well, this creates powerful long-term trust.' },
    LN12: { key: 'Playful Romantic',        body: 'Lightness and joy bring energy into your relationships. You enjoy laughter, spontaneity, and shared experiences that keep the connection vibrant. Emotional closeness grows through fun and playful interaction. With the right partner, love becomes both joyful and meaningful.' },
    LN13: { key: 'Quiet Idealist',          body: 'You hold a deep inner vision of what meaningful love should feel like. Because of this, you often seek connections that feel emotionally authentic and sincere. Surface-level relationships rarely satisfy you. When your ideals meet reality, love becomes deeply fulfilling.' },
    LN14: { key: 'Balanced Partner',        body: 'You naturally look for balance between independence and emotional closeness. Healthy relationships for you involve mutual respect and shared responsibility. Neither partner dominates the dynamic. This balance allows trust to grow steadily over time.' },
    LN15: { key: 'Reflective Romantic',     body: 'You approach love with introspection and emotional awareness. Relationships often inspire deep reflection about yourself and your partner. Because you value understanding, your connections grow stronger through honest conversation. This reflective quality often leads to meaningful emotional growth.' },
    LN16: { key: 'Passionate Idealist',     body: 'Love for you is something powerful and transformative. You seek relationships that inspire emotional intensity and shared dreams. When the connection feels meaningful, you invest deeply. With the right partner, your passion creates unforgettable bonds.' },
  };

  // ════════════════════════════════════════════════════════
  // SCORE MATRIX ENGINE — no threshold returns, accumulate + max
  // ════════════════════════════════════════════════════════

  function buildScoreMatrix() {
    const ln  = { LN1:0,LN2:0,LN3:0,LN4:0,LN5:0,LN6:0,LN7:0,LN8:0,LN9:0,LN10:0,LN11:0,LN12:0,LN13:0,LN14:0,LN15:0,LN16:0 };
    const rf  = {
      'Excitement Over Stability':0, 'Attracted to Emotional Distance':0, 'Expectation Mismatch':0,
      'Fear of Losing Independence':0, 'Overinvesting Too Early':0, 'Mixed Signal Tolerance':0,
      'Romanticizing Potential':0, 'Conflict Avoidance':0, 'Emotional Withdrawal':0,
      'Intensity Addiction':0, 'Fixer Instinct':0, 'Testing Loyalty':0,
      'Overthinking Signals':0, 'Attraction to Charisma':0, 'Seeking Emotional Validation':0,
      'Protecting Your Vulnerability':0, 'Fast Emotional Attachment':0, 'Avoiding Emotional Risk':0,
      'Attraction to Complexity':0, 'Idealizing the Beginning':0,
    };
    const hb  = { HB1:0,HB2:0,HB3:0,HB4:0,HB5:0,HB6:0,HB7:0,HB8:0,HB9:0,HB10:0,HB11:0,HB12:0,HB13:0,HB14:0,HB15:0,HB16:0 };
    const ly  = { LY1:0,LY2:0,LY3:0,LY4:0,LY5:0,LY6:0,LY7:0,LY8:0,LY9:0,LY10:0,LY11:0,LY12:0,LY13:0,LY14:0,LY15:0,LY16:0 };
    const sig = relSignals;

    // ── LOVE NATURE scoring ──────────────────────────────
    // dom1 base
    if (dom1 === 'O') { ln.LN4 += 30; ln.LN12 += 20; ln.LN7 += 15; }
    if (dom1 === 'P') { ln.LN1 += 30; ln.LN14 += 20; ln.LN7 += 10; }
    if (dom1 === 'W') { ln.LN8 += 30; ln.LN3 += 20; ln.LN15 += 10; }
    if (dom1 === 'A') { ln.LN11 += 30; ln.LN3 += 20; ln.LN8 += 10; }
    if (dom1 === 'R') { ln.LN2 += 30; ln.LN6 += 20; ln.LN13 += 10; }

    // intimacyStyle modifier
    if (intimacyStyle === 'warmly expressive' || intimacyStyle === 'unconventionally expressive') { ln.LN4 += 20; ln.LN12 += 10; }
    if (intimacyStyle === 'emotionally bonded') { ln.LN2 += 20; ln.LN6 += 10; }
    if (intimacyStyle === 'independence-oriented') { ln.LN1 += 20; ln.LN14 += 10; }
    if (intimacyStyle === 'commitment-focused') { ln.LN3 += 20; ln.LN11 += 10; }
    if (intimacyStyle === 'pragmatic & goal-aligned') { ln.LN8 += 15; ln.LN10 += 10; }
    if (intimacyStyle === 'intense but unstable') { ln.LN16 += 20; ln.LN7 += 10; }
    if (intimacyStyle === 'adaptable & measured') { ln.LN14 += 15; ln.LN5 += 10; }

    // score modifiers
    if (sig.intensity    > 70) { ln.LN16 += 20; ln.LN7 += 10; }
    if (sig.intensity    > 55 && sig.intensity <= 70) { ln.LN4 += 10; ln.LN12 += 8; }
    if (sig.idealism     > 70) { ln.LN13 += 25; ln.LN15 += 10; }
    if (sig.idealism     > 55 && sig.idealism <= 70) { ln.LN5 += 12; ln.LN15 += 8; }
    if (sig.stability    > 70) { ln.LN3 += 20; ln.LN11 += 10; }
    if (sig.stability    > 55 && sig.stability <= 70) { ln.LN8 += 10; ln.LN9 += 8; }
    if (sig.emotionalDepth > 70) { ln.LN2 += 15; ln.LN6 += 12; }
    if (sig.emotionalDepth > 55 && sig.emotionalDepth <= 70) { ln.LN9 += 10; ln.LN5 += 8; }
    if (sig.independence > 70) { ln.LN1 += 15; ln.LN14 += 10; }
    if (sig.independence > 55 && sig.independence <= 70) { ln.LN7 += 10; ln.LN10 += 8; }

    // clash / combine
    if (hasClash)   { ln.LN16 += 15; ln.LN7 += 10; }
    if (hasCombine) { ln.LN2 += 10; ln.LN9 += 10; }
    if (isShinYak)  { ln.LN6 += 10; ln.LN9 += 8; }

    // sangGwan (상관) 강하면 expressive/unconventional 부스트
    if (sangGwan > 0.6) { ln.LN4 += 10; ln.LN12 += 8; }

    // dom2 secondary boost (2위 에너지)
    if (dom2 === 'R') { ln.LN15 += 8; ln.LN5 += 5; }
    if (dom2 === 'O') { ln.LN4 += 8; ln.LN10 += 5; }
    if (dom2 === 'A') { ln.LN3 += 8; ln.LN11 += 5; }
    if (dom2 === 'W') { ln.LN8 += 8; ln.LN15 += 5; }
    if (dom2 === 'P') { ln.LN1 += 8; ln.LN14 += 5; }

    // ── RED FLAG scoring ─────────────────────────────────
    // LoveNature shadow (computed inline from ln scores)
    const lnTop = Object.entries(ln).sort((a,b) => b[1]-a[1])[0][0];

    const LN_RF_WEIGHTS = {
      LN1:  { 'Fear of Losing Independence':40, 'Attracted to Emotional Distance':15, 'Mixed Signal Tolerance':10 },
      LN2:  { 'Overinvesting Too Early':40, 'Fast Emotional Attachment':20, 'Seeking Emotional Validation':10 },
      LN3:  { 'Conflict Avoidance':35, 'Testing Loyalty':20, 'Expectation Mismatch':10 },
      LN4:  { 'Excitement Over Stability':35, 'Idealizing the Beginning':20, 'Intensity Addiction':10 },
      LN5:  { 'Overthinking Signals':35, 'Avoiding Emotional Risk':20, 'Protecting Your Vulnerability':10 },
      LN6:  { 'Fixer Instinct':35, 'Seeking Emotional Validation':20, 'Overinvesting Too Early':10 },
      LN7:  { 'Idealizing the Beginning':35, 'Excitement Over Stability':20, 'Attraction to Complexity':10 },
      LN8:  { 'Expectation Mismatch':35, 'Attraction to Charisma':15, 'Overthinking Signals':10 },
      LN9:  { 'Seeking Emotional Validation':35, 'Overinvesting Too Early':20, 'Fixer Instinct':10 },
      LN10: { 'Attraction to Complexity':35, 'Overthinking Signals':20, 'Mixed Signal Tolerance':10 },
      LN11: { 'Testing Loyalty':35, 'Conflict Avoidance':20, 'Expectation Mismatch':10 },
      LN12: { 'Excitement Over Stability':35, 'Idealizing the Beginning':15, 'Intensity Addiction':10 },
      LN13: { 'Romanticizing Potential':40, 'Idealizing the Beginning':20, 'Avoiding Emotional Risk':10 },
      LN14: { 'Mixed Signal Tolerance':35, 'Fear of Losing Independence':20, 'Avoiding Emotional Risk':10 },
      LN15: { 'Protecting Your Vulnerability':35, 'Romanticizing Potential':20, 'Overthinking Signals':10 },
      LN16: { 'Intensity Addiction':35, 'Attracted to Emotional Distance':20, 'Excitement Over Stability':10 },
    };
    const rfWeights = LN_RF_WEIGHTS[lnTop] || { 'Expectation Mismatch': 30 };
    Object.entries(rfWeights).forEach(([k, v]) => { if (rf[k] !== undefined) rf[k] += v; });

    // global modifiers
    if (hasClash)                   { rf['Intensity Addiction'] += 20; rf['Emotional Withdrawal'] += 10; }
    if (isShinYak)                  { rf['Overinvesting Too Early'] += 15; rf['Seeking Emotional Validation'] += 10; }
    if (dom1 === 'P' && sig.independence > 60) { rf['Fear of Losing Independence'] += 20; }
    if (dom1 === 'O' && sangGwan > 0.6)        { rf['Excitement Over Stability'] += 15; }
    if (sig.idealism > 70)          { rf['Romanticizing Potential'] += 15; }
    if (sig.emotionalDepth > 70 && !isShinYak) { rf['Attracted to Emotional Distance'] += 12; }
    if (rivalScore > 0.6)           { rf['Testing Loyalty'] += 15; }
    if (hasCombine)                 { rf['Fast Emotional Attachment'] += 15; }
    if (dom1 === 'R')               { rf['Protecting Your Vulnerability'] += 12; rf['Romanticizing Potential'] += 8; }
    if (dom1 === 'A')               { rf['Testing Loyalty'] += 12; rf['Expectation Mismatch'] += 8; }
    if (sig.stability > 65 && sig.intensity < 40) { rf['Conflict Avoidance'] += 15; }

    // ── HEARTBREAK scoring ───────────────────────────────
    if (dom1 === 'O') { hb.HB10 += 30; hb.HB13 += 20; hb.HB15 += 10; }
    if (dom1 === 'P') { hb.HB2  += 30; hb.HB11 += 20; hb.HB16 += 10; }
    if (dom1 === 'W') { hb.HB3  += 30; hb.HB12 += 20; hb.HB5  += 10; }
    if (dom1 === 'A') { hb.HB8  += 30; hb.HB4  += 15; hb.HB14 += 10; }
    if (dom1 === 'R') { hb.HB1  += 30; hb.HB9  += 20; hb.HB7  += 10; }

    // sub-type modifiers
    if (dom1 === 'O' && sangGwan > 0.6)           { hb.HB13 += 15; hb.HB6 += 10; }
    if (dom1 === 'W' && (scores['편재']||0) > 0.5) { hb.HB5  += 15; hb.HB3 += 10; }
    if (dom1 === 'A' && (scores['편관']||0) > 0.5) { hb.HB4  += 15; hb.HB8 += 10; }
    if (dom1 === 'R' && (scores['편인']||0) > 0.5) { hb.HB9  += 15; hb.HB1 += 10; }
    if (dom1 === 'P' && rivalScore > 0.6)          { hb.HB16 += 15; hb.HB2 += 10; }

    // global modifiers
    if (hasClash)                    { hb.HB4  += 25; hb.HB14 += 15; }
    if (isShinYak && sig.emotionalDepth > 65) { hb.HB7 += 20; hb.HB1 += 10; }
    if (hasCombine)                  { hb.HB11 += 15; }
    if (sig.intensity > 70)          { hb.HB4  += 15; hb.HB13 += 10; }
    if (sig.idealism > 65)           { hb.HB12 += 12; hb.HB16 += 10; }
    if (sig.independence > 60)       { hb.HB2  += 10; hb.HB5  += 8; }
    if (dom2 === 'O')                { hb.HB10 += 10; hb.HB6  += 8; }
    if (dom2 === 'A')                { hb.HB8  += 10; hb.HB14 += 8; }

    // ── LOVES YOU RIGHT scoring ──────────────────────────
    // stabilityArchetype 기반
    const saLabel = stabilityArchetype ? stabilityArchetype.label : '';
    const SA_LY = {
      'Emotionally Grounding':       { LY6:25, LY7:15 },
      'Quietly Reliable':            { LY11:25, LY1:15 },
      'Warm & Non-Competitive':      { LY9:25, LY12:15 },
      'Non-Controlling & Accepting': { LY12:25, LY2:15 },
      'Emotionally Intelligent':     { LY7:25, LY5:15 },
      'Expressive & Playful':        { LY13:25, LY4:15 },
      'Free-Spirited & Warm':        { LY2:25, LY13:15 },
      'Intellectually Stimulating':  { LY4:25, LY10:15 },
      'Expressive & Energizing':     { LY8:25, LY13:15 },
      'Supportive & Consistent':     { LY5:25, LY15:15 },
      'Ambitious & Energizing':      { LY8:25, LY14:15 },
      'Steady & Dependable':         { LY1:25, LY11:15 },
    };
    const lyWeights = SA_LY[saLabel] || { LY1:20 };
    Object.entries(lyWeights).forEach(([k, v]) => { ly[k] += v; });

    // dom1 complement
    if (dom1 === 'O') { ly.LY6 += 20; ly.LY5 += 10; }
    if (dom1 === 'P') { ly.LY1 += 15; ly.LY12 += 12; }
    if (dom1 === 'W') { ly.LY11 += 20; ly.LY14 += 10; }
    if (dom1 === 'A') { ly.LY3 += 20; ly.LY10 += 12; }
    if (dom1 === 'R') { ly.LY4 += 20; ly.LY5 += 15; }

    // global modifiers
    if (hasClash)                    { ly.LY6 += 20; ly.LY16 += 12; }
    if (hasCombine)                  { ly.LY15 += 20; ly.LY3 += 10; }
    if (sig.emotionalDepth > 70)     { ly.LY7 += 15; ly.LY9 += 10; }
    if (sig.stability > 70)          { ly.LY1 += 15; ly.LY11 += 10; }
    if (isShinYak && sig.idealism > 60) { ly.LY15 += 15; ly.LY9 += 10; }
    if (dom1 === 'R' && normR > 50)  { ly.LY5 += 12; ly.LY4 += 8; }
    if (dom1 === 'P')                { ly.LY2 += 12; }
    if (dom1 === 'A' && normA > 50)  { ly.LY10 += 12; ly.LY3 += 8; }
    if (sig.independence > 65)       { ly.LY12 += 12; ly.LY2 += 10; }
    if (sig.idealism > 65)           { ly.LY15 += 10; ly.LY13 += 8; }
    if (dom2 === 'R')                { ly.LY4 += 10; ly.LY5 += 8; }
    if (dom2 === 'A')                { ly.LY3 += 10; ly.LY1 += 8; }

    return { ln, rf, hb, ly, lnTop };
  }

  const _scoreMatrix = buildScoreMatrix();

  function maxKey(obj) {
    return Object.entries(obj).sort((a,b) => b[1]-a[1])[0][0];
  }

  const loveNatureId  = maxKey(_scoreMatrix.ln);
  const loveNatureObj = LN_TEMPLATES[loveNatureId] || LN_TEMPLATES['LN14'];

  const redFlagKey      = maxKey(_scoreMatrix.rf);
  // RF 템플릿 인라인 조회 (calcRelationalStructure 스코프)
  const _RF_INLINE = {
    'Excitement Over Stability':    { key: 'Excitement Over Stability',    body: 'You often feel drawn to relationships that begin with strong emotional excitement. The spark of intensity can make the connection feel meaningful very quickly. However, early excitement can sometimes overshadow whether the relationship offers long-term stability. Taking time to observe consistency helps you distinguish chemistry from compatibility.' },
    'Attracted to Emotional Distance': { key: 'Attracted to Emotional Distance', body: 'You may find yourself intrigued by people who seem emotionally hard to reach. Their independence or mystery can feel compelling at first. Over time this dynamic can leave you investing more emotional energy than you receive. Choosing partners who show openness and emotional availability often leads to healthier connections.' },
    'Expectation Mismatch':         { key: 'Expectation Mismatch',         body: 'You naturally imagine the deeper potential of a relationship early on. Because you look for meaning in connections, you may project possibilities that have not yet developed. When reality unfolds differently it can create subtle disappointment. Focusing on consistent actions rather than early impressions helps keep expectations balanced.' },
    'Fear of Losing Independence':  { key: 'Fear of Losing Independence',  body: 'Your independence is an important part of your identity. Because of this you may hesitate to fully lean into emotional closeness. Partners can sometimes interpret this as distance or uncertainty. Finding ways to maintain independence while allowing vulnerability strengthens relationships.' },
    'Overinvesting Too Early':      { key: 'Overinvesting Too Early',      body: 'When you feel emotionally connected you often give generously and quickly. This openness can create strong early bonds. However investing heavily before a relationship stabilizes can create imbalance. Allowing connections to unfold gradually helps build stronger foundations.' },
    'Mixed Signal Tolerance':       { key: 'Mixed Signal Tolerance',       body: 'You tend to be patient when someone\'s behavior is inconsistent. Because you try to understand people deeply you may overlook early mixed signals. Over time this can create confusion about where the relationship truly stands. Paying attention to patterns of consistency helps protect your emotional clarity.' },
    'Romanticizing Potential':      { key: 'Romanticizing Potential',      body: 'You naturally see the best possibilities in people. This means you sometimes connect with who someone could become rather than who they currently are. While this optimism is part of your emotional depth it can lead to waiting for change that never arrives. Recognizing the difference between potential and reality creates healthier expectations.' },
    'Conflict Avoidance':           { key: 'Conflict Avoidance',           body: 'You often value harmony and emotional peace. Because of this you may avoid difficult conversations in relationships. While this preserves short-term calm unresolved issues can quietly grow over time. Addressing small concerns early often strengthens trust.' },
    'Emotional Withdrawal':         { key: 'Emotional Withdrawal',         body: 'When emotions become overwhelming you may retreat inward. This helps you process feelings privately and regain balance. However partners may interpret this silence as emotional distance. Explaining your need for space helps maintain connection.' },
    'Intensity Addiction':          { key: 'Intensity Addiction',          body: 'You may feel most alive in relationships filled with emotional intensity. Passion and dramatic chemistry can feel deeply meaningful. Yet intensity alone does not always create stability. Learning to appreciate calm emotional rhythms can bring deeper fulfillment.' },
    'Fixer Instinct':               { key: 'Fixer Instinct',               body: 'You sometimes feel drawn to partners who need emotional support or guidance. Helping others grow can feel purposeful to you. However relationships built around fixing someone can become exhausting. Choosing partners who support you equally creates healthier balance.' },
    'Testing Loyalty':              { key: 'Testing Loyalty',              body: 'Trust matters deeply to you. Because of this you may unconsciously test how dependable someone is. Small situations may become ways to measure their commitment. While this instinct seeks security repeated testing can create pressure. Open communication builds trust more effectively.' },
    'Overthinking Signals':         { key: 'Overthinking Signals',         body: 'You tend to analyze emotional signals carefully. Messages tone and behavior may carry layers of meaning in your mind. While this awareness helps you notice subtle changes it can also create unnecessary worry. Looking at long-term patterns helps maintain perspective.' },
    'Attraction to Charisma':       { key: 'Attraction to Charisma',       body: 'You can feel strongly drawn to confident charismatic personalities. Their energy often creates immediate attraction. However strong charisma can sometimes hide deeper incompatibilities. Slowing down allows you to see whether emotional depth matches the initial spark.' },
    'Seeking Emotional Validation': { key: 'Seeking Emotional Validation', body: 'Feeling emotionally appreciated is important to you. Because of this you may gravitate toward partners who provide strong affirmation early on. While validation feels comforting relationships built mainly on reassurance can become unbalanced. Stability grows when emotional support flows both ways.' },
    'Protecting Your Vulnerability': { key: 'Protecting Your Vulnerability', body: 'Opening up emotionally can take time for you. Because vulnerability feels significant you may guard your deeper feelings carefully. Partners may struggle to understand your inner world early on. Gradually sharing emotional layers builds stronger bonds.' },
    'Fast Emotional Attachment':    { key: 'Fast Emotional Attachment',    body: 'When someone truly understands you emotional attachment can form quickly. This sense of recognition may make the relationship feel rare or special. However deep connection that develops too quickly can skip important stages of trust building. Allowing time for emotional foundations helps create stability.' },
    'Avoiding Emotional Risk':      { key: 'Avoiding Emotional Risk',      body: 'You sometimes hold back emotionally until you feel certain about a relationship. This instinct protects you from disappointment. Yet too much caution can prevent deeper connection from forming. Taking small emotional risks can strengthen intimacy.' },
    'Attraction to Complexity':     { key: 'Attraction to Complexity',     body: 'You may feel drawn to people with complicated emotional stories. Their depth can make relationships feel meaningful and transformative. However complexity can also bring unpredictability and instability. Simpler steadier dynamics often provide stronger foundations.' },
    'Idealizing the Beginning':     { key: 'Idealizing the Beginning',     body: 'The early stage of a relationship can feel magical to you. First impressions and initial chemistry may carry strong emotional weight. When relationships later settle into normal rhythms it may feel like something has faded. Recognizing that deeper love grows beyond the first spark helps adjust expectations.' },
  };
  const redFlagTemplate = _RF_INLINE[redFlagKey] || _RF_INLINE['Expectation Mismatch'];

  const heartbreakId  = maxKey(_scoreMatrix.hb);
  const _HB_INLINE = {
    HB1:  { key: 'The Emotionally Unavailable One',  body: 'Their self-contained presence feels mature and intriguing at first. The emotional distance reads as depth, and you find yourself working to earn their openness. But the warmth you sense never quite arrives on consistent terms. You end up giving more than you receive, and wondering whether closeness was ever really possible.' },
    HB2:  { key: 'The Freedom Seeker',               body: 'Their independence and easy confidence draw you in immediately. Being around them feels expansive and alive. But when the relationship asks for roots, they redirect to the horizon. You realize too late that what felt like freedom in them was actually unavailability for you.' },
    HB3:  { key: 'The Charming Opportunist',         body: 'Their charisma and warmth make everything feel effortless early on. They know exactly what to say and when to say it. But over time the effort shifts to you while their attention spreads outward. The connection that felt so mutual turns out to have been mostly one-directional.' },
    HB4:  { key: 'The Intense but Unstable One',     body: 'The immediate intensity between you feels rare and electric. Their passion makes you feel uniquely understood. But the same energy that creates this chemistry also drives their instability. What began as exciting quickly becomes exhausting as the emotional range refuses to settle.' },
    HB5:  { key: 'The Perpetual Wanderer',           body: 'Their stories and restless energy make life feel bigger. Being with them feels like joining a constant adventure. But no adventure ever ends at home. When you finally need consistency, you realize they were always planning the next departure.' },
    HB6:  { key: 'The Brilliant but Chaotic One',    body: 'Their intelligence and unconventional thinking are immediately magnetic. Conversations feel electric and ideas seem endless. But their inner world is rarely organized enough to sustain a relationship. You find yourself providing structure for someone who finds structure suffocating.' },
    HB7:  { key: 'The Validation Seeker',            body: 'They make you feel needed and chosen in a way that feels deeply fulfilling. Your support seems to matter enormously to them. But over time you realize they need constant reassurance that you cannot sustainably provide. The relationship becomes weighted by their emotional demands rather than nourished by mutual exchange.' },
    HB8:  { key: 'The Control-Oriented Partner',     body: 'Their directness and confidence feel like strength at the start. You appreciate someone who seems sure of where things are going. But what felt like direction gradually reveals itself as control. The relationship starts to narrow your world rather than expand it.' },
    HB9:  { key: 'The Mystery Lover',                body: 'Something about their opacity pulls you into constant interpretation. Every partial disclosure feels like progress and every silence becomes a puzzle. But mystery without eventual openness is just distance. You eventually realize you have been pursuing access to someone who was never fully present.' },
    HB10: { key: 'The Unpredictable Artist',         body: 'Their creativity and emotional range feel rare and exciting to be around. The world looks different through their perspective. But unpredictability as a relationship feature eventually becomes exhausting. You find yourself absorbing the instability that fuels their creative life.' },
    HB11: { key: 'The Commitment Avoider',           body: 'The connection feels genuine and the chemistry is real. Everything about the relationship feels right except the question of where it is going. They are present without being certain, engaged without committing. You realize the uncertainty was never temporary -- it was their permanent answer.' },
    HB12: { key: 'The Self-Focused Dreamer',         body: 'Their vision and ambition are genuinely inspiring. Being part of their world feels exciting and purposeful. But their focus rarely turns fully toward you for long. You end up being the support system for someone whose primary relationship is with their own potential.' },
    HB13: { key: 'The Dramatic Romantic',            body: 'Their emotional intensity makes everything feel cinematic and meaningful. The highs are extraordinary and their affection is overwhelming in the best way. But the lows come just as hard and just as often. The relationship requires emotional management that slowly drains the connection it was meant to build.' },
    HB14: { key: 'The Hot-and-Cold Partner',         body: 'Their inconsistency keeps you constantly recalibrating. When they are warm it feels like home. When they withdraw the contrast is disorienting. You spend more energy tracking their emotional temperature than building the relationship, and eventually realize that pattern was never going to change.' },
    HB15: { key: 'The Emotional Escape Artist',      body: 'They are present and emotionally accessible until things get real. Depth and difficulty send them redirecting toward humor, distraction, or absence. You keep waiting for the moment when they will stay through something hard. It rarely comes, and you are left holding the emotional weight of the relationship alone.' },
    HB16: { key: 'The Restless Visionary',           body: 'Their ambition and forward momentum are magnetic. They seem to be building toward something that matters. But the vision keeps expanding and the present moment keeps getting deprioritized. You realize you were a companion for a season of their journey rather than a destination in it.' },
  };
  const heartbreakObj = _HB_INLINE[heartbreakId] || _HB_INLINE['HB1'];

  const lovesYouRightId  = maxKey(_scoreMatrix.ly);
  const _LY_INLINE = {
    LY1:  { key: 'The Steady Builder',        body: 'This person builds things slowly and keeps their word. Their consistency is not exciting in the early days but it creates the kind of safety where you can finally stop bracing for disruption. They bring long-term thinking to a connection that needs it, and their reliability becomes the thing you cannot imagine going without.' },
    LY2:  { key: 'The Grounded Equal',        body: 'They match you without competing with you. There is a natural balance in how you move through the relationship -- neither person diminishing the other to make the dynamic work. Their groundedness does not ask you to shrink, and their confidence does not require your validation. It is rare and it works.' },
    LY3:  { key: 'The Loyal Protector',       body: 'Their loyalty runs deeper than most people realize until it has been tested. They are not loud about how much they care but they are consistent in ways that eventually become undeniable. You feel protected not because they remove difficulty but because they make it clear they are not going anywhere when it arrives.' },
    LY4:  { key: 'The Thoughtful Companion',  body: 'They notice things you did not know you needed noticed. Conversations with them feel like genuine exploration rather than performance. Their thoughtfulness is not a strategy -- it is just how they exist in a relationship. Over time you realize you have been building something with someone who was actually paying attention.' },
    LY5:  { key: 'The Patient Listener',      body: 'They do not rush you or redirect the conversation back to themselves. There is a quality of presence in them that makes it easier to say the things that are hardest to say. Their patience is not passivity -- it is an active form of care that creates the space you need to finally open up.' },
    LY6:  { key: 'The Calm Stabilizer',       body: 'When things get loud internally or externally, they remain settled. Their calm is not indifference -- it is a genuinely regulated emotional presence that steadies the energy around them. You stop mistaking this for low investment and start recognizing it as one of the most valuable things a partner can offer.' },
    LY7:  { key: 'The Emotionally Safe Partner', body: 'You do not have to manage how they will react before you speak. The absence of that calculation changes everything. They have built something in the relationship where honesty does not feel risky, and where you can be exactly where you are without performing stability you do not have. That safety is rare.' },
    LY8:  { key: 'The Supportive Strategist', body: 'They see your potential clearly and they know how to help you actually reach it. Their support is not generic encouragement -- it is thoughtful and specific and delivered at the right moment. Being with them makes you more capable, not more dependent, which is the difference between support and replacement.' },
    LY9:  { key: 'The Gentle Nurturer',       body: 'There is a softness in how they show up that does not make them weak -- it makes them safe. They notice when you are depleted before you say it. Their care is expressed in small consistent gestures that accumulate into something you start to count on. They replenish the relationship rather than draw from it.' },
    LY10: { key: 'The Honest Communicator',   body: 'They say what they mean and mean what they say. There is no decoding required and no maintenance of subtext. When something is difficult they address it directly but without cruelty. The honesty that initially feels vulnerable eventually becomes one of the primary reasons the relationship works at all.' },
    LY11: { key: 'The Reliable Planner',      body: 'They follow through. Not occasionally and not when it is easy -- consistently. Their reliability is not a performance of commitment, it is the actual substance of it. You eventually realize that the future feels more real with them because they are the kind of person who actually shows up to build it.' },
    LY12: { key: 'The Balanced Partner',      body: 'Neither person has to sacrifice the parts of themselves that matter most for this relationship to work. The balance between closeness and independence feels almost effortless with them. They bring perspective when you are in your head and presence when you are pulling away. The dynamic sustains because it does not ask too much from either side.' },
    LY13: { key: 'The Curious Explorer',      body: 'They approach you with genuine interest that does not fade once the early chapters are done. They want to understand how you think and what you care about and why -- and that curiosity extends into how you grow together over time. Being with them never feels entirely static because they keep showing up to discover more.' },
    LY14: { key: 'The Kind Realist',          body: 'They see you clearly -- not through the distortion of early projection or the deflation of disappointment -- and they stay. Their kindness is not naive. They know exactly who you are and what the relationship requires, and they choose it anyway. That particular combination of honesty and warmth is steadier than most romantic connections.' },
    LY15: { key: 'The Devoted Ally',          body: 'When things are hard they are on your side without needing to be asked. Their investment in your wellbeing is not conditional on whether things are going well. You start to understand that devotion at this level is not possession -- it is the quiet decision to keep showing up, made again and again without fanfare.' },
    LY16: { key: 'The Quiet Anchor',          body: 'Their presence stabilizes without demanding anything from you. You do not notice the anchoring effect immediately -- you notice it when they are absent and the drift begins. They do not hold you in place through expectation or obligation. They simply exist in a way that makes staying feel like the most natural thing.' },
  };
  const lovesYouRightObj = _LY_INLINE[lovesYouRightId] || _LY_INLINE['LY1'];
  ...
  return {
    ...
    redFlagKey:   redFlagTemplate.key,
    redFlagBody:  redFlagTemplate.body,
    loveNatureKey:      loveNatureObj.key,
    loveNatureBody:     loveNatureObj.body,
    heartbreakKey:      heartbreakObj.key,
    heartbreakBody:     heartbreakObj.body,
    lovesYouRightKey:   lovesYouRightObj.key,
    lovesYouRightBody:  lovesYouRightObj.body,
