...
//   META_PIXEL_ID       (= 1513677227103970)  — rate 상품 기본값
//   META_CAPI_TOKEN     (메타 이벤트매니저 > 데이터소스 > 설정 > 전환 API 액세스 토큰)
//   상품별로 데이터세트가 다르면 pixelId/capiToken 인자로 덮어쓴다 (출산택일 = fatelab.co 전용 세트)
...
async function sendMetaPurchase({ paymentId, value, contentName, contentId, email, ip, ua, fbp, fbc,
                                  pixelId, capiToken, sourceUrl, currency }) {
  const PIXEL = pixelId || process.env.META_PIXEL_ID;
  const TOKEN = capiToken || process.env.META_CAPI_TOKEN;
    ...
    const r = await fetch(`https://graph.facebook.com/v21.0/${PIXEL}/events?access_token=${TOKEN}`, {
      ...
      body: JSON.stringify({
        data: [{
          ...
          event_source_url: sourceUrl || 'https://sajublueprint.com/rate',
          user_data,
          custom_data: {
            value, currency: currency || 'KRW',
